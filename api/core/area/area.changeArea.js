    ...
    .spread((areasByDistance, lastAreasUser) => {
      ...
      lastAreasUser.forEach(function(lastArea) {
        areasByDistance.forEach(function(areaDistance) {
          ...
          if(areaDistance.id === lastArea.id && areaDistance.distance <= (areaDistance.radius + accuracy)) {
            // user still in this area, we just log it
            sails.log.info(`User ${location.user} still detected in area ${areaDistance.name}`);
          }
        });
      });

      // Then, we check the opposite. Did the user entered a new area ?
      // Firf we remove all the areas the user is in from array areaByDistance
      ...
      // Then for all areas the user was not in, we checked if the user just moved in (with strict distance verification (we don't take accuracy into account here)
      areasByDistance.forEach(function(areaDistance) {
