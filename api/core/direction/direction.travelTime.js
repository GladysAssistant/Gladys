...
module.exports = function travelTime(options){
  if(!options || !options.origin || !options.destination) {
    return Promise.reject(new Error('Direction : origin and destination options are required.')); 
  }
...
function getTravelTimeProvider(index, options){
  if(!shared.providers[index]) {
    return Promise.reject(new Error('No directions provider available')); 
  } 
