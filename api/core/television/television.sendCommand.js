...
function getDeviceType(params){
  if(params.device) {
    return gladys.utils.sqlUnique(queries.getDeviceTypeByDeviceId, [params.device]); 
  } else if(params.room) {
    return gladys.utils.sqlUnique(queries.getTelevisionDeviceTypeByRoom, [params.room]); 
  } else {

    // if user does not specify a room, or a device, we suppose there is only one 
    // and return this one

    return gladys.utils.sqlUnique(queries.getDefaultDeviceType, []); 
  }
}
