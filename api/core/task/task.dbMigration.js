...
module.exports = function(oldVersion) {
    ...
    if(!semver.lt(oldVersion, gladys.version)){
        sails.log.info('Gladys database schema/data is up to date. No migration required.');
    ...
    if(semver.lt(oldVersion, '3.7.4')) {
        ...
        return gladys.utils.sql(`DELETE FROM boxtype;`, [])
            ...
            .then(() => gladys.task.updateDbVersion('3.7.4'))
            .then(() => gladys.task.dbMigration('3.7.4'));
    }
};