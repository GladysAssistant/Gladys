...
module.exports = {
  ...
  exec: function(req, res, next) {
    gladys.deviceType
      .exec({ devicetype: req.params.id, value: req.body.value })
      .then(function(state) {
  ...
  execGet: function(req, res, next) {
    gladys.deviceType
      .exec({ devicetype: req.params.id, value: req.query.value })
      .then(function(state) {
        return res.json(state);
      })
      .catch(next);
  ...
  getInRoom: function(req, res, next) {
    gladys.deviceType
      .getByRoom({ room: req.params.id })
      .then(roomsDeviceTypes => {
        if (roomsDeviceTypes.length === 0) {
          return res.json({ id: req.params.id, deviceTypes: [] }); 
        }
  ...
  getDeviceTypeByCategory: function(req, res, next) {
    gladys.deviceType
      .getByCategory({ category: req.query.category, room: req.query.room, type: req.query.type })
