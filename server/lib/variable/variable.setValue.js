...
const { EVENTS, SYSTEM_VARIABLE_NAMES, normalizeMdnsHostname } = require('../../utils/constants');
...
async function setValue(key, value, serviceId = null, userId = null) {
  ...
  // the mDNS hostname is announced on the network as-is: it is validated and stored
  // normalized, so the saved value and the advertised one can never differ
  if (key === SYSTEM_VARIABLE_NAMES.MDNS_HOSTNAME) {
    valueToSave = normalizeMdnsHostname(value);
    if (valueToSave === null) {
      throw new BadParameters(`Invalid mDNS hostname "${value}"`);
    }
  }
  ...
  if (variable === null) {
    ...
    createdOrUpdatedVariable = await variable.update({ value: valueToSave });
  }
