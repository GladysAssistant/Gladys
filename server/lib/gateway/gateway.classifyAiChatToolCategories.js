...
- "weather": the outside weather at home: current conditions, outside temperature, rain, snow, wind, sun, sunrise or sunset, weather alerts, or a forecast for today, tomorrow or the coming days.
...
async function classifyAiChatToolCategories({ messageText, previousQuestions }) {
  ...
  try {
    // No model field on purpose: the purpose field tells the gateway this is
    // a classification request, and the gateway routes it to a fast model
    // (never the potentially large and slow model selected for the main
    // request). Model selection is a backend decision.
