...
module.exports = (actionsFunc) => {
   ...
   */
  async function executeAction(self, action, scope, path) {
   ...
   */
  async function executeActions(self, actions, scope, basePath = null) {
    ...
    await Promise.mapSeries(actions, async (parallelActions, columnIndex) => {
      ...
      await Promise.map(parallelActions, async (action, rowIndex) => {
        ...
        await executeAction(self, action, scope, path);
      });
    });
    return null;
  }
