...
const actionsFunc = {
  ...
  [ACTIONS.CONDITION.ONLY_CONTINUE_IF]: async (self, action, scope) => {
    ...
    action.conditions.forEach((condition) => {
      ...
      let currentValue;
      // An empty selector is still a device feature condition: it must abort with
      // DEVICE_FEATURE_NOT_FOUND, not fall back to the (undefined) variable branch
      if (condition.device_feature !== undefined) {
