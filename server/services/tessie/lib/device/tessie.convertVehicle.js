...
function convertVehicle(vehicleData) {
    ...
    const { vin, name, model: carModel, type, exteriorColor, year, version, batteryCapacity, batteryRangeMax, vehicle } = vehicleData;
    ...
    if (vehicle.last_state) {
        ...
        // Charging state
        const chargeFeatures = buildFeatureCharge(
            `tessie:${vin}`,
        ...
        // Climate features
        const climateFeatures = buildFeatureClimate(
            `tessie:${vin}`,
        ...
        // Command features
        const commandFeatures = buildFeatureCommand(
            `tessie:${vin}`,
        ...
        // Drive features
        const driveFeatures = buildFeatureDrive(
            `tessie:${vin}`,
        ...
        const consumptionFeatures = buildFeatureConsumption(
            `tessie:${vin}`,
            'ELECTRICAL_VEHICLE_CONSUMPTION'
        );

        // Vehicle state
        const vehicleStateFeatures = buildFeatureState(
            `tessie:${vin}`,
