...
} = require('./tessie.buildFeaturesSpecifEnergy');

/**
 * @description Transform Tessie Energy device to Gladys device.
 * @param {object} netatmoDevice - Tessie device.
 * @returns {object} Gladys device.
 * @example
 * tessie.convertDeviceEnergy({ ... });
 */
function convertDeviceEnergy(netatmoDevice) {
  ...
  logger.debug(`Tessie convert Energy device "${nameDevice}, ${model}"`);
  const features = [];
  let params = [];
  ...
  logger.info(`Tessie Energy device "${nameDevice}, ${model}" converted`);
  return device;
}
