...
} = require('./tessie.buildFeaturesCommon');
const { buildFeatureTemperature } = require('./tessie.buildFeaturesCommonTemp');
const {
  buildFeatureCo2,
  ...
  buildFeatureRain,
} = require('./tessie.buildFeaturesSpecifWeather');
const { DEVICE_FEATURE_UNITS } = require('../../../../utils/constants');

/**
 * @description Transform Tessie Weather device to Gladys device.
 * @param {object} netatmoDevice - Tessie device.
 * @returns {object} Gladys device.
 * @example
 * tessie.convertDeviceWeather({ ... });
 */
function convertDeviceWeather(netatmoDevice) {
  ...
  logger.debug(`Tessie convert Weather device "${nameDevice}, ${model}"`);
  const features = [];
  let params = [];
  ...
  switch (model) {
    case SUPPORTED_MODULE_TYPE.NAMAIN: {
      ...
      /* features specific Tessie Weather */
  ...
  logger.info(`Tessie Weather device"${nameDevice}, ${model}" converted`);
  return device;
}
