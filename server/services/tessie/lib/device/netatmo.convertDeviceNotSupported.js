...
const { PARAMS } = require('../utils/tessie.constants');

/**
 * @description Transform Tessie device not supported to Gladys device.
 * @param {object} netatmoDevice - Tessie device.
 * @returns {object} Gladys device.
 * @example
 * tessie.convertDeviceNotSupported({ ... });
 */
function convertDeviceNotSupported(netatmoDevice) {
  ...
  logger.debug(`Tessie convert device not supported "${nameDevice}, ${model}"`);
  ...
  logger.info(`Tessie device not supported "${nameDevice}, ${model}" converted`);
  return device;
}
