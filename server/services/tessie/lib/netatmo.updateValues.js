...
/**
 * @description Save values of an Tessie device.
 * @param {object} deviceGladys - Device object in Gladys.
 * @param {object} deviceNetatmo - Device object coming from the Tessie API.
...
async function updateValues(deviceGladys, deviceNetatmo, externalId) {
  ...
  if (prefix !== 'tessie') {
    throw new BadParameters(`Tessie device external_id is invalid: "${externalId}" should starts with "tessie:"`);
  }
  if (!topic || topic.length === 0) {
    throw new BadParameters(`Tessie device external_id is invalid: "${externalId}" have no id and category indicator`);
  }
  if (!reachable && typeof reachable !== 'undefined') {
    logger.info(`Tessie device "${deviceGladys.name}" is not reachable`);
  }
