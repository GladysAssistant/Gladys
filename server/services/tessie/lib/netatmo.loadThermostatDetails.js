...
const { API, SUPPORTED_CATEGORY_TYPE } = require('./utils/tessie.constants');

/**
 * @description Discover Tessie cloud thermostats.
...
async function loadThermostatDetails() {
  ...
  try {
    ...
    if (!responseGetThermostat.ok) {
      logger.error('Tessie error: ', responseGetThermostat.status, rawBody);
    }
