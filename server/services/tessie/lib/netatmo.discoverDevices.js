...
/**
 * @description Discover Tessie cloud devices.
 * @returns {Promise} List of discovered devices;.
 * @example
 * await discoverDevices();
 */
async function discoverDevices() {
  logger.debug('Looking for Tessie devices...');
  if (this.status !== STATUS.CONNECTED) {
    await this.saveStatus({ statusType: this.status, message: null });
    throw new ServiceNotConfiguredError('Unable to discover Tessie devices until service is not well configured');
  }
  ...
  try {
    ...
    logger.info(`${devicesNetatmo.length} Tessie devices found`);
  } catch (e) {
  ...
  if (devicesNetatmo.length > 0) {
    ...
    logger.debug(`${discoveredDevices.length} new Tessie devices found`);
