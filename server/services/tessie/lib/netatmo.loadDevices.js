...
async function loadDevices() {
  ...
  if (this.configuration.energyApi || (!this.configuration.energyApi && !this.configuration.weatherApi)) {
    try {
      ...
      if (!responsePage.ok) {
        logger.error('Tessie error: ', responsePage.status, rawBody);
      }
  ...
  logger.debug(`${listDevices.length} Tessie devices loaded`);
  logger.info(`Tessie devices not supported : ${listDevices.filter((device) => device.not_handled).length}`);
  return listDevices;
}
