...
async function discover() {
  ...
  console.log(discoveredDevices.length);
  if (discoveredDevices.length) {
    await Promise.map(discoveredDevices, async (discoveredDevice) => {
      ...
      logger.debug(`--------------NOM : "${discoveredDevice.packet.answers[1].rdata.model_number}`);
      ...
      if (discoveredDevice.packet.answers[1].rdata.model_number === 'DSP-W215'){
        ...
        let deviceId = `${W215_EXTERNAL_ID_BASE}:${discoveredDevice.address}`;
        ...
        if (deviceInGladys){
          ...
          unknownDevices.push(models['DSP_W215'].getDevice(this.serviceId, discoveredDevice));
        }
      } else {
