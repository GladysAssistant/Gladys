 ...
 * @returns {Object} Return POW device.
 * @example
...
function getDevice(serviceId, device, channel = 0) {
  ...
  return {
    ...
    features: [
      ...
      {
        name: `${name} Power`,
        external_id: `${DEVICE_EXTERNAL_ID_BASE}:${device.deviceid}:${channel}:${DEVICE_FEATURE_TYPES.SWITCH.POWER}`,
        selector: `${DEVICE_EXTERNAL_ID_BASE}:${device.deviceid}:${channel}:${DEVICE_FEATURE_TYPES.SWITCH.POWER}`,
        category: DEVICE_FEATURE_CATEGORIES.SWITCH,
