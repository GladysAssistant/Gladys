...
async function installMqttContainer() {
  logger.info(`MQTT container is already installed by default`);
  ...
  if (dockerContainers.length === 0) {
    ...
    try {
      ...
      const brokerEnv = await exec(`sh ./server/services/zwavejs2mqtt/docker/gladys-zwave-mqtt-env.sh ${basePathOnHost}`);
      logger.trace(brokerEnv);
      containerDescriptorToMutate.HostConfig.Binds.push(`${basePathOnHost}/mqtt:/mosquitto/config`);
