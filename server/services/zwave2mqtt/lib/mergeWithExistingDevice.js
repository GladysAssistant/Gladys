 ...
 * @param {Object} zwave2mqttDevice - Discovered device.
 * @returns {Object} Device merged with Gladys existing one.
 * @example mergeWithExistingDevice(discorveredDevice)
 */
function mergeWithExistingDevice(zwave2mqttDevice) {
  const existing = this.gladys.stateManager.get('deviceByExternalId', zwave2mqttDevice.external_id);
  if (existing) {
    const device = { ...existing, ...zwave2mqttDevice };
