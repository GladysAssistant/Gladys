...
async function calculateProductionFromIndexFromBeginning(jobId) {
  return queueWrapper(this.queue, async () => {
    ...
    logger.info(`Calculating production from index from beginning in timezone ${systemTimezone}`);
    ...
    // Filter devices that have both INDEX and THIRTY_MINUTES_PRODUCTION features
    ...
    productionDevices.forEach((productionDevice) => {
      const indexFeatures = productionDevice.features.filter(
        (f) =>
          PRODUCTION_INDEX_FEATURE_TYPES[f.category] && PRODUCTION_INDEX_FEATURE_TYPES[f.category].includes(f.type),
      );

      const productionFeatures = productionDevice.features.filter(
        (f) =>
      ...
      if (indexFeatures.length > 0 && productionFeatures.length > 0) {
    ...
    await Promise.each(devicesWithBothFeatures, async (deviceWithBothFeatures) => {
      ...
      logger.debug(`Destroying last production index processed for ${deviceWithBothFeatures.device.id}`);
      await this.gladys.device.destroyParam(deviceWithBothFeatures.device, ENERGY_PRODUCTION_INDEX_LAST_PROCESSED);
    });

    logger.info(
      `Found ${devicesWithBothFeatures.length} devices with both INDEX and THIRTY_MINUTES_PRODUCTION features`,
    );
    ...
    await Promise.each(windows, async (windowTime) => {
      try {
        // Call the existing calculateProductionFromIndex function for each window
        await this.calculateProductionFromIndex(windowTime);
        successfulWindows += 1;
