...
module.exports = function EnergyMonitoringController(energyMonitoringHandler) {
  ...
  /**
   * @api {post} /api/v1/service/energy-monitoring/calculate-cost-range Calculate cost on a date range
  ...
  async function calculateCostRange(req, res) {
    ...
    energyMonitoringHandler.calculateCostRange(startDate, featureSelectors, endDate);
    res.json({
      success: true,
    });
  }
   ...
   * Calculate consumption from index on a date range
  ...
  async function calculateConsumptionFromIndexRange(req, res) {
    ...
    const startDate = getOptionalDate(req.body, 'start_date');
    const endDate = getOptionalDate(req.body, 'end_date');
    energyMonitoringHandler.calculateConsumptionFromIndexRange(startDate, featureSelectors, endDate);
    res.json({
      success: true,
    });
  }
