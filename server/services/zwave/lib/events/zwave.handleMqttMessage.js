...
const { driverReady } = require('./zwave.driverReady');
const { statisticsUpdated } = require('./zwave.statisticsUpdated');
const { valueUpdated } = require('./zwave.valueUpdated');
const { valueAdded } = require('./zwave.valueAdded');
const { nodeDead, nodeAlive, nodeWakeUp, nodeSleep } = require('./zwave.nodeState');
 ...
 * @returns {Object} Null.
 * @example
 * handleMqttMessage('zwavee2mqtt/POWER', 'ON');
 */
function handleMqttMessage(topic, message) {
  ...
  switch (topic) {
    case `${DEFAULT.ROOT}/_EVENTS/${DEFAULT.ZWAVE2MQTT_CLIENT_ID}/driver/driver_ready`: {
      const msg = JSON.parse(message).data[0];
      this.driver.controller.homeId = msg.homeId;
      this.driver.controller.ownNodeId = msg.controllerId;
      driverReady.bind(this)(msg.homeId);
      break;
    }
    case `${DEFAULT.ROOT}/_EVENTS/${DEFAULT.ZWAVE2MQTT_CLIENT_ID}/driver/all_nodes_ready`: {
      scanComplete.bind(this)();
      break;
    }
    case `${DEFAULT.ROOT}/_EVENTS/${DEFAULT.ZWAVE2MQTT_CLIENT_ID}/controller/statistics_updated`: {
      const msg = JSON.parse(message).data[0];
      this.driver.controller.statistics = msg;
      break;
    }
    case `${DEFAULT.ROOT}/_EVENTS/${DEFAULT.ZWAVE2MQTT_CLIENT_ID}/node/node_alive`: {
      const msg = JSON.parse(message).data[0];
      nodeAlive.bind(this)(
        {
          id: msg.id,
    ...
    case `${DEFAULT.ROOT}/_EVENTS/${DEFAULT.ZWAVE2MQTT_CLIENT_ID}/node/node_ready`: {
      const msg = JSON.parse(message).data[0];
      nodeReady.bind(this)(
        {
          id: msg.id,
    ...
    case `${DEFAULT.ROOT}/_EVENTS/${DEFAULT.ZWAVE2MQTT_CLIENT_ID}/node/node_sleep`: {
      const msg = JSON.parse(message).data[0];
      nodeSleep.bind(this)({
        id: msg.id,
      });
    ...
    case `${DEFAULT.ROOT}/_EVENTS/${DEFAULT.ZWAVE2MQTT_CLIENT_ID}/node/node_dead`: {
      const msg = JSON.parse(message).data[0];
      nodeDead.bind(this)(
        {
          id: msg.id,
    ...
    case `${DEFAULT.ROOT}/_EVENTS/${DEFAULT.ZWAVE2MQTT_CLIENT_ID}/node/node_wakeup`: {
      const msg = JSON.parse(message).data[0];
    ...
    case `${DEFAULT.ROOT}/_EVENTS/${DEFAULT.ZWAVE2MQTT_CLIENT_ID}/node/node_value_added`: {
      // Use node topic
      break;
    }
    case `${DEFAULT.ROOT}/_EVENTS/${DEFAULT.ZWAVE2MQTT_CLIENT_ID}/node/node_value_updated`: {
      // Use node topic
      break;
    }
    case `${DEFAULT.ROOT}/_EVENTS/${DEFAULT.ZWAVE2MQTT_CLIENT_ID}/node/node_metadata_updated`: {
      // Use node topic
      /* const msg = JSON.parse(message).data[0];
    ...
    case `${DEFAULT.ROOT}/_EVENTS/${DEFAULT.ZWAVE2MQTT_CLIENT_ID}/node/statistics_updated`: {
    ...
    case `${DEFAULT.ROOT}/_CLIENTS/${DEFAULT.ZWAVE2MQTT_CLIENT_ID}/status`:
    case `${DEFAULT.ROOT}/_CLIENTS/${DEFAULT.ZWAVE2MQTT_CLIENT_ID}/version`: {
      break;
    }
    ...
    default: {
      ...
      if (splittedTopic[1] === '_CLIENTS') {
        ...
        logger.info(`Zwave2mqtt scan in progress. Bypass message.`);
      } else if (splittedTopic.length >= 5) {
        const [, nodeId, commandClass, endpoint, propertyName, propertyKey] = splittedTopic;
        if (propertyKey === 'set') {
          // logger.debug(`Zwave2mqtt set. Bypass message.`);
          break;
        }
        if (GENRE[commandClass * 1] !== undefined) {
          // logger.debug(`Zwave2mqtt command class not supported. Bypass message.`);
          break;
        }
        ...
        logger.debug(`Zwave2mqtt topic ${topic} not handled.`);
      }
    }
  }
  return null;
