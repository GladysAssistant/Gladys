 ...
 * @example
 * state('tasmota:sonoff-plug', '{"key": "value"}', []);
 */
function state(deviceExternalId, message, events) {
  ...
  events.push({
    device_feature_external_id: `tasmota:${deviceExternalId}:${DEVICE_FEATURE_CATEGORIES.SWITCH}:${DEVICE_FEATURE_TYPES.SWITCH.BINARY}`,
