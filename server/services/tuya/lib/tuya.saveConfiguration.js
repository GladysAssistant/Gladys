...
async function saveConfiguration(configuration) {
  ...
  const { endpoint, accessKey, secretKey, appAccountId, serverTuyaEnv } = configuration;
  ...
  await this.gladys.variable.setValue(GLADYS_VARIABLES.TUYA_SERVER_ENVIRONMENT, serverTuyaEnv, this.serviceId);
