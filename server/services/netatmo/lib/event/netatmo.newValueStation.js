...
function newValueStation(data) {
  ...
  if (data.type === 'NAMain') {
    logger.debug(`Netatmo : New station, sid = ${sid} - ${data.type}`);
    const newSensor = {
    ...
    if (data.modules !== undefined) {
      data.modules.forEach((module) => {
        if (
          module.data_type[0] === 'Rain' ||
          module.data_type[0] === 'Wind' ||
          (module.data_type[0] === 'Temperature' &&
            (module.data_type[1] === 'Humidity' || module.data_type[1] === 'CO2'))
        ) {
          const sidModule = module._id;
          const moduleName = module.module_name;
          logger.debug(`Netatmo : New Module station, sid = ${sidModule} - ${module.data_type}`);
          let newSensor2;
          if (module.data_type[0] === 'Wind') {
            newSensor2 = {
              ...
              model: `netatmo-${module.type}-${module.data_type}`,
              should_poll: false,
              features: [
                {
          ...
          if (module.data_type[0] !== 'Rain' && module.data_type[0] !== 'Wind') {
            if (module.data_type.length === 2) {
              newSensor2 = {
                ...
                selector: `netatmo:${sidModule}`,
                external_id: `netatmo:${sidModule}`,
                model: `netatmo-${module.type}-outdoor`,
                should_poll: false,
                features: [
                  {
          ...
          if (module.data_type.length === 3) {
            newSensor2 = {
              ...
              selector: `netatmo:${sidModule}`,
              external_id: `netatmo:${sidModule}`,
              model: `netatmo-${module.type}-indoor`,
              should_poll: false,
              features: [
                {
                  name: `Temperature - ${moduleName}`,
          ...
          logger.info(`Files newValueStation - Module type unknown : ${module.data_type}`);
        }
      });
    }
    this.addSensor(sid, newSensor);
  } else {
    logger.info(`Files newValueStation - Device type unknown : ${data.type}`);
  }
}
