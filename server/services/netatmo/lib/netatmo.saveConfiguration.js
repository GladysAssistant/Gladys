...
async function saveConfiguration(configuration) {
  ...
  const { clientId, clientSecret, energyApi, weatherApi, securityApi } = configuration;
  try {
    ...
    await this.gladys.variable.setValue(GLADYS_VARIABLES.SECURITY_API, securityApi, this.serviceId);
    ...
    this.configuration.securityApi = securityApi;
