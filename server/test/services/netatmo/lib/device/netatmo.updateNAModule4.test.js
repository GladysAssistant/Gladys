...
describe('Netatmo update Smart Indoor module NAModule4 features', () => {
  const deviceGladysNAModule4 = devicesGladys[4];
  const deviceNetatmoNAModule4 = JSON.parse(JSON.stringify(devicesNetatmo[4]));
  const externalIdNAModule4 = `netatmo:${devicesNetatmo[4].id}`;
  beforeEach(() => {
    sinon.reset();
