...
describe('Netatmo update NAModule3 features', () => {
  const deviceGladysNAModule3 = devicesGladys[4];
  const deviceNetatmoNAModule3 = JSON.parse(JSON.stringify(devicesNetatmo[4]));
  const externalIdNAModule3 = `netatmo:${devicesNetatmo[4].id}`;
  beforeEach(() => {
    sinon.reset();
