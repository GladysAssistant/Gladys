...
describe('Netatmo Convert Device', () => {
  ...
  describe('Netatmo Convert Device', () => {
    it('should correctly convert a Netatmo Plug device', () => {
      ...
      const gladysDevice = convertDevice.call(NetatmoHandlerMock, deviceMock);
    ...
    it('should correctly convert a Netatmo device without room and without modules_bridged', () => {
      ...
      const paramMock = gladysDevice.params.filter((param) => param.name === 'firmware_revision')[0];
      expect(paramMock).to.have.property('value', deviceMock.firmware_revision);
