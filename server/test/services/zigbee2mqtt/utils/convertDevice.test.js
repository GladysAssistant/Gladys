...
describe('Zigbee2mqtt - Utils - convertDevice', () => {
  ...
  it('matching XIAOMI WXKG01LM', async () => {
    ...
    expect(result.features[0]).to.not.have.property('id');
    expect(result.features[0]).to.have.property('external_id', 'zigbee2mqtt:name:action');
    expect(result.features[0]).to.have.property('selector', 'zigbee2mqtt:name:action');
  });
