...
describe('TuyaHandler.buildLocalScanResponse', () => {
  ...
  it('should build local devices list when no discovered devices array', () => {
    const { buildLocalScanResponse } = proxyquire('../../../../services/tuya/lib/tuya.localScan', {});
    const stateManagerGet = sinon
      .stub()
      .callsFake((_, externalId) => (externalId === 'tuya:device2' ? { external_id: externalId } : null));
    const tuyaManager = {
    ...
    const response = buildLocalScanResponse(tuyaManager, {
      devices: {
        device1: { ip: '1.1.1.1', version: '3.3' },
        device2: { ip: '2.2.2.2', version: '3.3' },
      },
      portErrors: {},
    });
