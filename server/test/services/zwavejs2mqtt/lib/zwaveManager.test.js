...
describe('zwavejs2mqttManager commands', () => {
  ...
  it('should addNode', () => {
    ...
    assert.calledWithExactly(zwavejs2mqttManager.mqttClient.publish, `${DEFAULT.ROOT}/_CLIENTS/${DEFAULT.ZWAVEJS2MQTT_CLIENT_ID}/api/startInclusion/set`);
    
    clock.tick(ADD_NODE_TIMEOUT);
    expect(zwavejs2mqttManager.scanInProgress).to.equal(true);
    assert.calledWithExactly(zwavejs2mqttManager.mqttClient.publish, `${DEFAULT.ROOT}/_CLIENTS/${DEFAULT.ZWAVEJS2MQTT_CLIENT_ID}/api/stopInclusion/set`);
    assert.calledWithExactly(zwavejs2mqttManager.mqttClient.publish, `${DEFAULT.ROOT}/_CLIENTS/${DEFAULT.ZWAVEJS2MQTT_CLIENT_ID}/api/getNodes/set`, 'true');
    clock.restore();
  });
  ...
  it('should removeNode', () => {
    ...
    assert.calledWithExactly(zwavejs2mqttManager.mqttClient.publish, `${DEFAULT.ROOT}/_CLIENTS/${DEFAULT.ZWAVEJS2MQTT_CLIENT_ID}/api/startExclusion/set`);
