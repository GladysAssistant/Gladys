...
describe('TasmotaService - Model - Neo Coolcam', () => {
  it('get model for Neo Coolcam', () => {
    const model = models[modelId].getModel();

    expect(model).to.eq('neo-coolcam');
  });

  it('get label for Neo Coolcam', () => {
    const label = models[modelId].getLabel();

    expect(label).to.eq('Neo Coolcam');
  });

  it('get features for Neo Coolcam', () => {
