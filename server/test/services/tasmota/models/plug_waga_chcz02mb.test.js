...
describe('TasmotaService - Model - WAGA CHCZ02MB', () => {
  it('get model for WAGA CHCZ02MB', () => {
    const model = models[modelId].getModel();

    expect(model).to.eq('waga-chcz02mb');
  });

  it('get label for WAGA CHCZ02MB', () => {
    const label = models[modelId].getLabel();

    expect(label).to.eq('WAGA CHCZ02MB');
  });

  it('get features for WAGA CHCZ02MB', () => {
