...
describe('TasmotaService - Model - Sonoff Pow R2', () => {
  it('get model for Sonoff Pow R2', () => {
    const model = models[modelId].getModel();

    expect(model).to.eq('sonoff-pow-r2');
  });

  it('get label for Sonoff Pow R2', () => {
    const label = models[modelId].getLabel();

    expect(label).to.eq('Sonoff Pow R2');
  });

  it('get features for Sonoff Pow R2', () => {
