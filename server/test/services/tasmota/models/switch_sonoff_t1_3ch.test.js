...
describe('TasmotaService - Model - Sonoff T1 3Ch', () => {
  it('get model for Sonoff T1 3Ch', () => {
    const model = models[modelId].getModel();

    expect(model).to.eq('sonoff-t1-3ch');
  });

  it('get label for Sonoff T1 3Ch', () => {
    const label = models[modelId].getLabel();

    expect(label).to.eq('Sonoff T1 3Ch');
  });

  it('get features for Sonoff T1 3Ch', () => {
