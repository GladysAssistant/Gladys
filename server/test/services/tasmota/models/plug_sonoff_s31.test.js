...
describe('TasmotaService - Model - Sonoff S31', () => {
  it('get model for Sonoff S31', () => {
    const model = models[modelId].getModel();

    expect(model).to.eq('sonoff-s31');
  });

  it('get label for Sonoff S31', () => {
    const label = models[modelId].getLabel();

    expect(label).to.eq('Sonoff S31');
  });

  it('get features for Sonoff S31', () => {
