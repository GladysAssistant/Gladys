...
describe('Matter.convertToGladysDevice', () => {
  ...
  it('should create a light brightness feature for LevelControl lighting cluster', async () => {
    const clusterClient = {
      ...
      getMinLevelAttribute: async () => 10,
      getMaxLevelAttribute: async () => 200,
    };
  ...
  it('should fallback to default min/max for LevelControl lighting when attributes throw', async () => {
    const clusterClient = {
      id: LevelControl.Complete.id,
      name: 'LevelControl',
      endpointId: 4,
  ...
  it('should create television features for MediaPlayback cluster', async () => {
    ...
    expect(gladysDevice.features.map((f) => f.type)).to.deep.equal(['play', 'pause', 'stop']);
    expect(gladysDevice.features.map((f) => f.category)).to.deep.equal(['television', 'television', 'television']);
  });
  ...
  it('should create television keypad features for KeypadInput cluster', async () => {
    ...
    expect(gladysDevice.features.map((f) => f.type)).to.deep.equal(['up', 'down', 'left', 'right', 'enter', 'return']);
    expect(gladysDevice.features.every((f) => f.category === 'television')).to.equal(true);
  });
});
