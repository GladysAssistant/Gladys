...
describe('zWave getMode Utils', () => {
    ...
    it('should return READ ONLY', () => {
        ...
        expect(mode).equals(VALUE_MODES.READ);
        expect(mode&VALUE_MODES.WRITE).equals(0);
    });

    it('should return WRITE ONLY', () => {
        const mode = getMode(writeOnlyValue);

        expect(mode).equals(VALUE_MODES.WRITE);
        expect(mode&VALUE_MODES.READ).equals(0);
    });

    it('should return READ WRITE', () => {
        expect(getMode(readWriteValue)).equals(VALUE_MODES.READ|VALUE_MODES.WRITE);
    });
})