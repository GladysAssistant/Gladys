...
describe('Integration host API', () => {
  ...
  describe('communication: /message and /contact', () => {
    const JOHN_USER_ID = '0cd30aef-9c4e-4a23-88e3-3547971296e5';
    let communicationService;
    let communicationToken;
    let originalHandleEvent;

    beforeEach(async () => {
      communicationService = await seedExternalService({
        name: 'ext-dev-signal-bridge',
        selector: 'ext-dev-signal-bridge',
        manifest: { ...TEST_MANIFEST, type: 'communication', config_schema: undefined },
        has_message_feature: true,
      });
      communicationToken = generateIntegrationToken(communicationService.id, 1, 'secret');
      // keep the brain out of these HTTP tests: the event emission is what
      // we assert, not the whole chat pipeline
      originalHandleEvent = gladys.message.handleEvent;
      gladys.message.handleEvent = () => Promise.resolve(null);
    });

    afterEach(() => {
      gladys.message.handleEvent = originalHandleEvent;
    });

    it('should link a contact with a code generated by the user, then receive messages', async () => {
      const { code } = await gladys.externalIntegration.createLinkCode('ext-dev-signal-bridge', JOHN_USER_ID);
      const linkRes = await integrationRequest(communicationToken)
        .post('/api/integration/v1/contact/link')
        .send({ code, contact_id: 'signal-12345', contact_name: 'John on Signal' })
        .expect('Content-Type', /json/)
        .expect(200);
      expect(linkRes.body).to.deep.equal({
        user: { selector: 'john', first_name: 'John', language: 'en' },
      });
      await integrationRequest(communicationToken)
        .post('/api/integration/v1/message')
        .send({ contact_id: 'signal-12345', text: 'Turn on the light' })
        .expect(200)
        .then((res) => {
          expect(res.body).to.deep.equal({ success: true });
        });
    ...
    it('should return 404 on an invalid code and on an unknown contact', async () => {
      await integrationRequest(communicationToken)
        .post('/api/integration/v1/contact/link')
        .send({ code: 'WRONG123', contact_id: 'signal-12345' })
        .expect(404);
      await integrationRequest(communicationToken)
        .post('/api/integration/v1/message')
        .send({ contact_id: 'signal-12345', text: 'hello' })
        .expect(404);
    });

    it('should not let an integration use the link code of another one', async () => {
      // the code is bound to the OTHER integration (default seeded service)
      const { code } = await gladys.externalIntegration.createLinkCode(service.selector, JOHN_USER_ID);
      await integrationRequest(communicationToken)
        .post('/api/integration/v1/contact/link')
