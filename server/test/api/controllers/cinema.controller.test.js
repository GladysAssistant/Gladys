...
const { fake } = sinon;
const CinemaController = require('../../../api/controllers/cinema.controller');
...
describe('CinemaController', () => {
  ...
  describe('getUpcoming', () => {
    it('should get upcoming movies in the user language', async () => {
      const gladys = {
        cinema: { getUpcoming: fake.resolves(expectedMovies) },
      };
      const req = { query: {}, user: { language: 'fr' } };
      const res = { json: fake.returns(null) };
      const cinemaController = CinemaController(gladys);
      await cinemaController.getUpcoming(req, res);
      expect(gladys.cinema.getUpcoming.firstCall.args[0]).to.deep.equal({
        language: 'fr',
        region: undefined,
        daysAhead: NaN,
      });
    ...
    it('should pin the provider chosen in the widget configuration', async () => {
      ...
      const cinemaController = CinemaController(gladys);
      await cinemaController.getUpcoming(req, res);
      expect(gladys.cinema.getUpcoming.firstCall.args[0]).to.have.property('service', 'tmdb');
    });
    it('should not forward an empty service query parameter', async () => {
      ...
      const cinemaController = CinemaController(gladys);
      await cinemaController.getUpcoming(req, res);
      expect(gladys.cinema.getUpcoming.firstCall.args[0]).to.not.have.property('service');
    });
  });
  ...
  describe('getProviders', () => {
    it('should list the providers with their integration label', async () => {
      const gladys = {
        cinema: { getProviders: fake.returns(['ext-tvdb', 'tmdb']) },
        externalIntegration: {
      ...
      const res = { json: fake.returns(null) };
      const cinemaController = CinemaController(gladys);
      await cinemaController.getProviders(req, res);
