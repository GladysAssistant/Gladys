...
const DEFAULTS = { default_mode: 'heating', control_type: 'hysteresis' };
...
export const loadDeviceConfig = async (httpClient, thermostatFeature) => {
  ...
  try {
    const devices = await httpClient.get('/api/v1/device', { device_feature_selectors: thermostatFeature });
    paramsConfig = buildConfigFromParams(devices && devices[0]);
  } catch (e) {
    paramsConfig = null;
  }
