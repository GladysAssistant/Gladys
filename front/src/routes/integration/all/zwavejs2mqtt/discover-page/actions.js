...
const createActions = store => {
  ...
  const actions = {
    async getNodes(state) {
      ...
      try {
        const zwaveNodes = await state.httpClient.get('/api/v1/service/zwavejs2mqtt/node');
    ...
    async addNode(state, e, secure = false) {
      ...
      try {
        await state.httpClient.post('/api/v1/service/zwavejs2mqtt/node/add', {
          secure
        });
        store.setState({
    ...
    async stopAddNode(state) {
      ...
      try {
        await state.httpClient.post('/api/v1/service/zwavejs2mqtt/cancel');
        store.setState({
    ...
    async refreshValues(state, device) {
      await state.httpClient.post('/api/v1/service/zwavejs2mqtt/values/refresh', {
        nodeId: device.rawZwaveNode.id
      });
    },
    async refreshInfo(state, device) {
      await state.httpClient.post('/api/v1/service/zwavejs2mqtt/info/refresh', {
        nodeId: device.rawZwaveNode.id
      });
    },
