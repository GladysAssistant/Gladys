...
import XiaomiSensorMagnetBox from './XiaomiSensorMagnetBox';
import { RequestStatus } from '../../../../../utils/consts';
import style from '../style.css';

class XiaomiSensorMagnet extends Component {
  constructor(props) {
    super(props);
              ...
              <select class="form-control" onChange={this.updateSensorChoice}>
                ...
                {props.sensorMagnet && (
                  <optgroup label={'sensor'}>
                    {props.sensorMagnet.map((sensor, index) => (
        ...
        <div class="card-body">
          <div class={cx('dimmer-content', style.xiaomiListBody)}>
            <div class="row my-4 mx-2">
              {props.xiaomiSensorMagnet &&
                props.xiaomiSensorMagnet.map((sensor, index) => (
                  <XiaomiSensorMagnetBox
                    sensor={sensor}
                    sensorIndex={index}
                    updateSensorField={props.updateSensorField}
                    updateNameField={props.updateNameField}
                    saveSensor={props.saveSensor}
                    deleteSensor={props.deleteSensor}
                    houses={props.houses}
                  />
                ))}
              {props.xiaomiSensorMagnet && props.xiaomiSensorMagnet.length === 0 && (
                <div class="col-md-12">
...
export default XiaomiSensorMagnet;
