...
class HeatzyDeviceSetupPage extends Component {
  selectFeature(e) {
    this.setState({
  ...
  updateFeatureProperty(e, property, featureIndex) {
    ...
    if (property === 'external_id' && !value.startsWith('heatzy:')) {
      if (value.length < 5) {
        value = 'heatzy:';
      } else {
        value = `heatzy:${value}`;
  ...
  async componentWillMount() {
    ...
    await this.props.getIntegrationByName('heatzy');
...
export default HeatzyDeviceSetupPage;
