...
const buildIssueBody = (device, localPollStatus, localPollError, localPollValidation, localPollDps) => {
  ...
  sections.push(
    ...
    '',
    'If this device needs new support in Gladys, the maintainer will usually need to:',
    '- create or update a cloud mapping file in `server/services/tuya/lib/mappings/cloud/`',
    '- create or update a local mapping file in `server/services/tuya/lib/mappings/local/`',
    '- register the mapping in `server/services/tuya/lib/mappings/index.js`',
    '- re-run conversion and polling expectations after the mapping changes',
    '',
    'Note:',
    '',
