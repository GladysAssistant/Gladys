...
class SetupTab extends Component {
  ...
  async getConfiguration() {
    ...
    try {
      const { value: username } = await this.props.httpClient.get(
        '/api/v1/service/melcloud-home/variable/MELCLOUD_HOME_USERNAME'
      );
      melCloudUsername = username;

      const { value: password } = await this.props.httpClient.get(
        '/api/v1/service/melcloud-home/variable/MELCLOUD_HOME_PASSWORD'
      );
  ...
  saveConfiguration = async e => {
    ...
    try {
      await this.props.httpClient.post('/api/v1/service/melcloud-home/variable/MELCLOUD_HOME_USERNAME', {
        value: this.state.melCloudUsername.trim()
      });

      await this.props.httpClient.post('/api/v1/service/melcloud-home/variable/MELCLOUD_HOME_PASSWORD', {
        value: this.state.melCloudPassword.trim()
      });

      // start service
      await this.props.httpClient.post('/api/v1/service/melcloud-home/start');
      this.setState({
    ...
    return (
      <div class="card">
        <div class="card-header">
          <h1 class="card-title">
            <Text id="integration.melcloudHome.setup.title" />
          </h1>
        </div>
        <div class="card-body">
          <div
          ...
          >
            ...
            <div class="dimmer-content">
              <p>
                <MarkupText id="integration.melcloudHome.setup.description" />
              </p>

              <form>
                <div class="form-group">
                  <label for="melCloudUsername" class="form-label">
                    <Text id={`integration.melcloudHome.setup.username`} />
                  </label>
                  <Localizer>
                    <input
                      name="melCloudUsername"
                      type="text"
                      placeholder={<Text id="integration.melcloudHome.setup.usernamePlaceholder" />}
                ...
                <div class="form-group">
                  <label htmlFor="melCloudPassword" className="form-label">
                    <Text id={`integration.melcloudHome.setup.password`} />
                  </label>
                  <div class="input-icon mb-3">
                    <Localizer>
                      <input
                        ...
                        placeholder={<Text id="integration.melcloudHome.setup.passwordPlaceholder" />}
                ...
                <div class="row mt-5">
                  <div class="col">
                    <button type="submit" class="btn btn-success" onClick={this.saveConfiguration}>
                      <Text id="integration.melcloudHome.setup.saveLabel" />
                    </button>
                  </div>
                </div>
              </form>
