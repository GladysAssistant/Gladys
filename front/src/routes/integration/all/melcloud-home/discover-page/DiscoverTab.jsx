...
import MELCloudHomeDeviceBox from '../MELCloudHomeDeviceBox';
...
class DiscoverTab extends Component {
  ...
  getDiscoveredDevices = async () => {
    ...
    try {
      const discoveredDevices = await this.props.httpClient.get('/api/v1/service/melcloud-home/discover');
      this.setState({
        discoveredDevices,
        loading: false,
        errorLoading: false
    ...
    return (
      <div class="card">
        <div class="card-header">
          <h1 class="card-title">
            <Text id="integration.melcloudHome.discover.title" />
          </h1>
          <div class="page-options d-flex">
            <button onClick={this.getDiscoveredDevices} class="btn btn-outline-primary ml-2" disabled={loading}>
              <Text id="integration.melcloudHome.discover.scan" /> <i class="fe fe-radio" />
            </button>
          </div>
        </div>
        <div class="card-body">
          <div class="alert alert-secondary">
            <Text id="integration.melcloudHome.discover.description" />
          </div>
          <div
            class={cx('dimmer', {
              active: loading
            ...
            <div class={cx('dimmer-content', style.melcloudListBody)}>
              {errorLoading && (
                <p class="alert alert-warning">
                  <Text id="integration.melcloudHome.status.notConnected" />
                  <Link href="/dashboard/integration/device/melcloud-home/setup">
                    <Text id="integration.melcloudHome.status.setupPageLink" />
                  </Link>
                </p>
              )}
              <div class="row">
