...
const actions = store => {
  const actions = {
    async getCurrentZwave2mqttUrl(state) {
      ...
      try {
        const zwave2mqttUrl = await state.httpClient.get(
          '/api/v1/service/zwave2mqtt/variable/ZWAVE2MQTT_URL'
        );
        store.setState({
    ...
    async getStatus(state) {
      store.setState({
        zwave2mqttGetStatusStatus: RequestStatus.Getting
      });
      try {
        const zwave2mqttStatus = await state.httpClient.get('/api/v1/service/zwave2mqtt/status');
        store.setState({
          zwave2mqttStatus,
          zwave2mqttGetStatusStatus: RequestStatus.Success
        });
      } catch (e) {
        store.setState({
          zwave2mqttGetStatusStatus: RequestStatus.Error
        });
      }
    }
  };
