import { Text } from 'preact-i18n';

const EditActions = props => (
  <div class="fixed-bottom footer">
    <div class="container">
      <div class="row align-items-center flex-row-reverse">
        <div class="col-auto">
          <button onClick={props.cancelDashboardEdit} class="btn btn-outline-secondary btn-sm ml-2">
            <Text id="dashboard.editDashboardCancelButton" /> <i class="lucide icon-slash" />
          </button>
          <button onClick={props.deleteCurrentDashboard} class="btn btn-outline-danger btn-sm ml-2">
            <Text id="dashboard.editDashboardDeleteButton" /> <i class="lucide icon-trash" />
          </button>
          <button onClick={props.saveDashboard} class="btn btn-outline-primary btn-sm ml-2">
            <Text id="dashboard.editDashboardSaveButton" /> <i class="lucide icon-check" />
          </button>
        </div>
      </div>
    </div>
  </div>
);

export default EditActions;
