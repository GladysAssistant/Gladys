...
class EditBoxColumns extends Component {
  constructor(props) {
    super(props);
    this.columnCardRef = createRef();
    this.state = {
      parentHeight: 0,
      maxBoxes: 3
    };
  ...
  updateHeight = () => {
    if (this.props.homeDashboard.boxes.length < this.state.maxBoxes && this.columnCardRef.current) {
      const children = this.columnCardRef.current.children;
      let maxHeight = 0;
  ...
  render() {
    ...
    const maxBoxes = this.state.maxBoxes;
    ...
    const addButtonStyle = {
      ...
      '--add-button-height': `${this.state.parentHeight}px`
    };
    return (
      <div class="pb-6">
        <h3>
        ...
        <DndProvider backend={props.isTouchDevice ? TouchBackend : HTML5Backend}>
          ...
          <div ref={this.columnCardRef} class={cx('d-flex flex-row align-items-start', style.columnCard)}>
            {props.homeDashboard &&
              ...
              <div class={cx('d-flex flex-column pl-2', style.addButton)} style={addButtonStyle}>
                ...
                <Localizer>
                  <button
                    ...
                    onClick={() => props.addColumn(boxesLength)}
