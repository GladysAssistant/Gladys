describe('Withings settings page', () => {
  ...
  it('Check setting page and connect', () => {
    ...
    cy.get('.markup')
      .i18n('integration.oauth2.instructions');

    cy.get('.form-label')
      .i18n('integration.oauth2.apiKeyLabel');
    ...
    cy.get('.alert-info') 
      .i18n('integration.withings.settings.complete');

    cy.get('.alert-info') 
    .i18n('integration.withings.settings.clientId');

    cy.get('.alert-info').contains('FakeClientId');

    cy.get('.alert-info') 
      .i18n('integration.withings.settings.afterIntegration');

    cy.get('.alert-info') 
      .i18n('integration.withings.settings.instructionsToUse');

    cy.contains('p', 'integration.oauth2.delete') 
      .should('exist');
