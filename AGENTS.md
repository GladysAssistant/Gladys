...
- **External integrations** (`docs/specs/external-integrations/`, entry point `README.md`): supervisor, host API, integration WebSocket protocol, manifest, store formats, SDK contract. The spec is a folder of topic files — `core/` (the framework), `contracts/` (the v1 contracts, C.1–C.8), `capabilities/` (one self-contained file per integration type or capability: communication, network discovery, Gladys Plus webhooks, weather, calendar) — and the folder listing is its index. Any PR that changes a behavior or contract of external integrations must update the file that owns it **in the same diff**. A **new integration type or capability is a new file `capabilities/<topic>.md`** that owns its whole design and its contracts (manifest field, endpoints, WebSocket messages, SDK methods): never rows appended to the shared `contracts/` tables, never a per-file map in the README, never a renumbering — historic section identifiers (B.x, C.x) stay in the headings that have one, and no new one is allocated. The calendar type (`capabilities/calendar-type.md`, B.19) has shipped its milestone 1; its per-user OAuth2 milestone is design-only. Implement from the spec, not from scratch.
