...
- **External integrations** (`docs/specs/external-integrations/`, entry point `README.md`): supervisor, host API, integration WebSocket protocol, manifest, store formats, SDK contract. The spec is split into **one file per section** (`b01-data-model.md` … `b20-image-cleanup.md`, `c01-manifest.md` … `c08-js-sdk-api.md`); section identifiers (B.x, C.x) are stable and must never be renumbered. Any PR that changes a behavior or contract of external integrations must update the file that owns it **in the same diff**. A **new integration type or capability gets its own new file** (`b21-<topic>.md`, next free number) on the model of B.15–B.18, plus a row in the README map — never a new chapter appended to an existing file. Phase-2 designs (B.15 communication, B.16 network discovery, B.17 Gladys Plus webhooks, B.18 weather) each live in their own file — implement from the spec, not from scratch.
