...
- **House alarm modes** (`docs/specs/house-alarm-modes.md`): the arming modes of the house alarm — the five values of `t_house.alarm_mode`, the settable subset, the events, websocket messages and REST routes that go with them, the single delay before arming, panic as an action, and the HomeKit mapping. The enumeration is append-only; any PR that changes those values or contracts must update the spec in the same diff.
- **House alarm codes** (`docs/specs/house-alarm-codes.md`): the alarm codes of an instance — one bcrypt-hashed code per person plus admin-created guest codes with an expiry, the uniqueness rule and the rate limits that protect it, the `t_alarm_code` model, the REST contract, and the `user` / `user_name` fields the alarm events carry. Any PR that changes one of those behaviors or contracts must update the spec in the same diff.
