...
- **Water heater** (`docs/specs/water-heater.md`): the `water-heater` device feature category — its six types, the `WATER_HEATER_MODE` enum, how per-appliance mode subsets are declared through `supported_options`, the frontend registration points and the external-integration contract. Designed under `device-feature-categories.md`, whose checklist it answers in its section D. The core and frontend are implemented; the integration mappings (Matter, Zigbee2MQTT) are not — update the spec in the same diff as any change to those types or their semantics.
