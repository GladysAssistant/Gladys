...
- Transport : param `GLADYS_TRANSPORT` validé (`local|cloud|unreachable`, clé `GLADYS_*` non définie → `400`), `POST /device/transport` (batch, param mis à jour, push WS `device-transport-updated`, external_id inconnu ignoré), interrupteur `GLADYS_PREFER_LOCAL` (rendu si `transports: ["local","cloud"]`, stocké, poussé via `config-updated`, refusé en écriture sur le `POST /config` intégration → `400`).
...
**`POST /api/integration/v1/device/transport`** — corps `{ "transports": [ { "device_external_id": "ext:tuya-demo:plug1", "transport": "local" } ] }` (batch ≤ 100) → `200 { "success": true }`. Met à jour le param `GLADYS_TRANSPORT` des devices concernés **sans re-publier la liste découverte** (le chemin léger pour les bascules à chaud : le lien cloud saute → `unreachable`, le LAN revient → `local`), et pousse `device-transport-updated` au front (macarons en temps réel). Valeur hors `local|cloud|unreachable` → `400` ; un `device_external_id` inconnu est ignoré silencieusement.
...
| `publishTransports(transports)` | batch `POST /device/transport` — `[{ external_id, transport: 'local' \| 'cloud' \| 'unreachable' }]` (macarons, cf. C.3) ; la préférence utilisateur arrive dans `gladys.config.GLADYS_PREFER_LOCAL` |
