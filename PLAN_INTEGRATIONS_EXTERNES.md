...
- **Accès matériel** : le manifeste demande des **classes d'accès nommées**, jamais des chemins : en v1 `coral-usb` (`/dev/bus/usb`), `coral-pcie` (`/dev/apex_*`), `gpu` (`/dev/dri`), `video` (`/dev/video*`). Affichées à l'install ; une classe absente de l'hôte n'est simplement pas montée (le conteneur démarre sans, à l'intégration de dégrader proprement — Frigate repasse en détection CPU). Un chemin arbitraire (`/dev/sda`…) est impossible par construction.
...
| **Configuration** | `.../config` | **Formulaire généré depuis le `config_schema` JSON du manifeste** (UI déclarative de la RFC : champs texte/nombre/booléen/select/secret en v1, aucun code injecté), sauvegarde via `POST .../config`. S'y ajoutent le bloc de supervision : statut, actions start/stop/restart, modal logs, désinstaller (admin) ; pour les intégrations multi-conteneurs, l'état de chaque sous-conteneur, un sélecteur de conteneur dans la modal logs, et un lien « Ouvrir <label> » par port publié (ex. l'UI de Frigate, `http://<hôte-gladys>:<port assigné>`) |
...
- Sous-conteneurs : validation du champ `containers` (bornes, noms uniques, env `GLADYS_*` interdit, volumes absolus, classes `devices` inconnues rejetées, `ports` bornés), descripteurs + réseau privé sur une fixture type Frigate + Mosquitto (DockerodeMock), `start` auto vs manual, état désiré (conteneur exité « censé tourner » → restart avec backoff ; arrêté via l'API → laissé arrêté), assignation de port hôte (libre, persisté à travers les recréations), résolution des classes matérielles (présentes/absentes sur l'hôte), endpoints `/container` (liste, start avec `env` → recréation si diff, stop, restart, `404` sur nom inconnu, **isolation : le token de A ne pilote pas les conteneurs de B**), désinstallation complète (conteneurs + réseau privé + ports libérés + dossier data supprimés), réconciliation des orphelins au boot.
...
- `devices` : classes d'accès matériel nommées, parmi `coral-usb` | `coral-pcie` | `gpu` | `video` (liste v1, extensible par version de schéma — jamais de chemin `/dev` libre) ;
...
```
— les sous-conteneurs déclarés, leur état Docker, l'état désiré (B.2) et les ports hôte assignés ; liste vide si l'intégration n'en déclare pas.
...
| `GET .../:selector` | → détail (mêmes champs, + `"containers": [ { "name", "status", "ports": [{ "container_port", "host_port", "label" }] } ]` pour les multi-conteneurs — le front en tire les liens « Ouvrir ») |
...
| Périphériques | `Devices` uniquement via les classes déclarées : `coral-usb` → `/dev/bus/usb`, `coral-pcie` → `/dev/apex_*`, `gpu` → `/dev/dri`, `video` → `/dev/video*` — résolues par le superviseur, montées seulement si présentes sur l'hôte |
...
6. **Jalon 6 — Sous-conteneurs** : validation du champ `containers` (dont ports et classes matérielles), réseau privé par intégration, état désiré/santé/réconciliation dans le superviseur, assignation des ports hôte, résolution des classes matérielles, endpoints `/container` (liste/start/stop/restart), front (ports et matériel sur l'écran d'installation, états + sélecteur de logs + liens « Ouvrir » dans le bloc de supervision) + tests sur fixture type Frigate + Mosquitto. → une intégration multi-conteneurs s'installe, expose son UI, accède au Coral, et se désinstalle sans rien laisser derrière elle.
