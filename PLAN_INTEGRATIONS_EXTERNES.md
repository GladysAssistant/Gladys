...
- **Admin** : `POST /api/v1/external_integration` avec **trois modes d'installation** — `{ store_slug }` (depuis le store, le serveur résout image + manifeste depuis son cache d'index), `{ repo_url }` (**depuis une URL de repo GitHub**, indexé ou non : le serveur résout la branche par défaut via l'API GitHub, télécharge `gladys-assistant-integration.json` en raw, le valide avec le même JSON Schema que l'indexeur, puis suit le chemin d'install standard ; `store_slug` est déduit = `owner/repo`), `{ docker_image, manifest }` (mode dev sans repo). Puis `POST .../:selector/update` (pull + recréation du conteneur), `POST .../start|stop|restart`, `GET .../logs`, `DELETE` (`?delete_devices=true`).
