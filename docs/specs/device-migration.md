...
- **Philips Hue, TP-Link and Sonos are flagged `deprecated`** in the front catalog (`front/src/config/integrations/devices.json`) and their pages mount the existing `DeprecationWarning` banner, like Netatmo/MELCloud/Tuya.
- **Device-level references are rewritten unconditionally, so the destination must cover them.** A `play_notification` scene action stores the **device** selector (not a feature selector): `device.migrate` rewrites it whatever the feature mapping says, and the action then looks up `MUSIC.PLAY_NOTIFICATION` on the destination at run time. The mapping table cannot express this, so the modal shows a dedicated warning when the source exposes `MUSIC.PLAY_NOTIFICATION` and the destination does not (B.5), and a replacement integration must implement that feature before its Migrate button ships. Native Sonos ("talk on a speaker" scenes) is the case that surfaced this: the community [gladys-sonos](https://github.com/vincentBesseau/gladys-sonos) replacement implements it since v1.1.0.
...
6. **`t_energy_price.electric_meter_device_id` re-point** and **`t_energy_contract.electric_meter_device_id` re-point** (section A; the overlap check of the contracts throws `ConflictError` and stops the run) then **room inheritance** (rule of section A); on room change, refresh the destination device in caches and `notify(EVENTS.DEVICE.UPDATE)`.
...
- **Mounted on the device boxes** of the deprecated integrations: `netatmo/NetatmoDeviceBox.jsx`, `melcloud/MELCloudDeviceBox.jsx`, `tuya` device box, `philips-hue/device-page/Device.jsx`, `tp-link/device-page/Device.jsx`, `zwavejs-ui/ZwaveJSUIDeviceBox.jsx`, `sonos/SonosDeviceBox.jsx` — **only** on already-created devices (not on discovery items).
- **Modal flow**: (1) pick the destination device — the modal loads `get /api/v1/device` and filters out the source device and its whole service; candidates are grouped by integration (keyed on the integration's slug, labelled with the same translated/disambiguated name as `/dashboard/devices`, via `getDeviceIntegration`/`disambiguateIntegrationNames` from `front/src/routes/devices/integrationLinks.js`) and each option is labelled with its room (or "no room"), so same-named devices from different integrations/rooms stay distinguishable; the dropdown's text filter matches on integration, room and device name (accent/case-insensitive, `front/src/utils/normalizeSearchText.js`); (2) feature mapping table pre-filled by the auto-matching (B.1), one row per source feature, each row a dropdown of remaining destination features plus "do not migrate"; unmapped rows, `type` mismatches and `unit` mismatches (values are moved without conversion) show a warning; a device-level warning is shown when the source exposes `MUSIC.PLAY_NOTIFICATION` and the destination does not (a `play_notification` scene action is re-pointed to the destination regardless of the mapping, and would fail at run time); the confirm button is disabled while a migration is in flight (no double-submit); (3) recap ("history, scenes and dashboards will be migrated; the old device will be deleted") → confirm → spinner → success screen with the report counts, then the integration's device list refreshes. Network failure shows a "migration may still be running, check the jobs page" message (B.4).
- **Catalog**: `deprecated: true` added to `philipsHue`, `tpLink` and `sonos` in `front/src/config/integrations/devices.json`; `DeprecationWarning` mounted in `PhilipsHuePage.jsx`, `TpLinkPage.jsx` and `SonosPage.jsx` (first child of the `col-lg-9` column, existing pattern).
