...
- **Feature parity is checked before a migrate button ships, not left to the mapping UI.** `device.migrate` rewrites a `play_notification` scene action's **device** selector unconditionally, regardless of whether the destination has a matching feature — the unmapped-feature warning only covers feature-level references, not this device-level one. Native Sonos implements `MUSIC.PLAY_NOTIFICATION` ("talk on a speaker" scenes); the community [gladys-sonos](https://github.com/vincentBesseau/gladys-sonos) replacement did not when its Migrate button first shipped, which would have silently broken such scenes after a migration. Closed in gladys-sonos v1.1.0 (`@svrooij/sonos`'s own `PlayNotification()`, same defaults as native) before merge, rather than documented as a gap — see that repo's `docs/en.md`/`docs/fr.md` for the one remaining, unavoidable UX difference (no early revert: the integration's container cannot receive the speaker's UPnP "playback stopped" event the way native, host-networked, can).
