...
2. **`type` stays a technical key.** It keeps driving routing (`/dashboard/integration/{type}/{key}`), the screen set (device screens vs configuration-only), the host-API surface, and the non-admin visibility rules (`HIDDEN_CATEGORIES_FOR_NON_ADMIN_USERS` remains expressed on `type`). Four technical types: `device`, `communication`, `weather`, and `provider` — an integration with no device surface whose whole contract is the capability fields it declares (dashboard widgets first; see `external-integrations/capabilities/dashboard-widgets.md`, section 1, which also states the rule: a new capability is a manifest field usable by every type, never a new type). `provider` is hidden from non-admins like `device` and `weather`. Browse categories are **display metadata**, decoupled from URLs and behavior — this is what makes the whole redesign low-risk and free of any data migration.
