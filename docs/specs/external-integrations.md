...
| **2** | Mediated network discovery (passive listening **and active scan** broadcast/mDNS/SSDP by the core — full design in B.16), inbound webhooks via Gladys Plus (generic gateway → integration relay, full design in B.17), advanced config widgets, device-scoped actions (`scope: "device"`, button on the device card — in v1 the `source: "devices"` select covers the need, see C.1), integration types other than "Devices" — first the "communication" type to move Telegram & co out of the core (full design in B.15), the "ai" type to plug community AI providers in place of Gladys Plus (B.18, shipped), then weather…. | An integration detects its hardware without manual config; a messaging channel installs from the store. |
...
### B.18 "AI provider" type: pluggable AI backends (implemented)
...
- **Proxy service capability `ai.chat(request)`** (the B.15 extension pattern: `device.*`, `message.*`, now `ai.*`): relay over WS `external-integration.ai.chat { message_id, request }` with the standard `command-result` ack carrying the completion in `data` (C.4). Ack deadline **120 s** (third documented exception to the 5 s rule, after `camera.get-image` and per-action timeouts): reasoning models are slow, and one request can carry a full tool-calling context. A non-object `data` → `ExternalIntegrationUnavailableError` (the standard AI failure reply, never a crash).
...
| `type` | string | yes | `"device"` \| `"communication"` (B.15) \| `"ai"` (B.18) |
...
| core → integration | `external-integration.ai.chat` | `{ "message_id": "uuid", "request": { "messages": [...], "tools": [...], "tool_choice": "auto"\|"required", "purpose": "chat"\|..., "categories": [...] } }` — an AI request routed to the integration because it is the selected AI provider (`type: "ai"`, B.18). `request` is an **OpenAI-compatible chat completion request** (the `model` field is stripped by the core: the provider picks its own model); respond via `command-result` with `data` = an **OpenAI-compatible completion** (`choices[0].message` with `content` / `tool_calls`); ack expected within **120 s** (reasoning models are slow — third exception to the 5 s rule) |
...
| `DELETE .../:selector` *(admin)* | → `200 { "success": true }` — removes **everything**: container, devices, config variables, `t_service` row (explicit confirmation in the UI); if the integration was the selected AI provider (B.18), the `AI_PROVIDER` variable is cleared (back to Gladys Plus) |

**AI provider selection (B.18)** — dedicated literal routes (outside `/external_integration`, the choice includes Gladys Plus which is not an external integration):
...
| `onAiChat(cb)` | `(request) => Promise<object>` — handler of the `ai.chat` relay (`type: "ai"` integrations, B.18): receives an OpenAI-compatible chat completion request, resolves an OpenAI-compatible completion (it goes into `data`; 120 s ack delay, see C.4) |
