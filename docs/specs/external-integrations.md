...
> **Living specification — source of truth.** This document specifies the behavior of external integrations and the contracts binding the monorepo to the ecosystem repos (section C: manifest, host API, WS protocol, store formats). Phase 1 is implemented, and the phase 2 workstreams of sections B.15–B.18 (communication type, mediated network discovery, inbound webhooks, weather type) have shipped since; B.19 is reserved for the AI provider type specified in a parallel PR; B.20 (Docker image cleanup) and B.21 (TTS provider type) are implemented. **Rule: any PR that changes an external-integration behavior or contract modifies this file in the same diff** — spec first, code second. Field feedback (pilot ports, forum) is captured here before being coded.
...
- TTS provider (B.21): manifest `type: "tts"` accepted (`messaging`/`contact_schema` still rejected outside communication), `tts.synthesize` relay (audio in `command-result.data`, **30 s** timeout honored, curated content types — own-key lookup, inherited object properties rejected —, 5 MB decoded bound, 2000-character text cap, malformed audio → throw), `gladys.tts` dispatch (variable set → provider proxy with defense-in-depth payload validation; absent → Gladys Plus path untouched; uninstalled/stopped provider → throw, no silent fallback), consumer failure contract (voice assistant: a Gladys Plus `403`/`429` still emits the `forbidden`/`too_many_requests` upsell error and rethrows, any other TTS failure returns the transcription and the answer with `ttsUrl: null` and emits no error; scene `MUSIC.PLAY_NOTIFICATION`: still throws, contained by the scene framework), token audio route (200 with the curated content type, 404 unknown/expired, RAM cache purged lazily), `GET`/`POST /api/v1/tts/provider` (list with display names, `422` on an unavailable provider, admin-only write, non-admin read), uninstall of the selected provider clears `TTS_ACTIVE_PROVIDER`.
...
### B.21 "TTS provider" type (implemented)
...
- A configured provider that is stopped or disconnected → `getSpeechUrl` **throws**: a silent fallback to another voice would be a surprise, not a service. What each consumer does with that failure differs, because what they can still deliver differs:
  - **Gladys Plus `403` / `429`** (no subscription, quota reached) are **never swallowed**, on either consumer: they are the existing upsell paths (B.13) and keep surfacing as the `forbidden` / `too_many_requests` voice-assistant errors and as the scene error.
  - **Any other TTS failure on the voice-assistant path** (`gateway.processVoiceMessage`) **degrades to a text-only reply**: the transcription and the answer are already computed and already emitted over the websocket — discarding them behind a generic error would lose a usable answer to a missing voice. The response carries `ttsUrl: null` and the failure is logged.
  - **The `MUSIC.PLAY_NOTIFICATION` scene action still throws**: there is no URL to hand to the speaker, so there is nothing to degrade to. The scene framework (`scene.executeActions.js`) already contains and logs the failure, and the scene continues.
...
- **Manifest**: `type: "tts"`, nothing else specific — voice, model, speed, per-language voices… all belong to the integration's own `config_schema` (generated form), where the choices are engine-specific by nature. Same screens as a communication integration: **Configuration only** (no Devices/Discovery), technical type `tts`; nothing is exposed to non-admins (`404`, indistinguishable from an unknown selector — a TTS provider has no per-user block). A store TTS manifest declares **`categories: ["assistants"]`** (C.1, vocabulary of `integration-catalog-categories.md` §3) — the whole type belongs on the "Voice assistants & AI" shelf, and an undeclared provider would sit in the uncategorized `All`/search-only bucket; the legacy `/dashboard/integration/tts` path redirects to that shelf (§5 of the same spec).
...
| `type` | string | yes | `"device"` \| `"communication"` (B.15) \| `"weather"` (B.18) \| `"tts"` (B.21) |
...
| `DELETE .../:selector` *(admin)* | → `200 { "success": true }` — removes **everything**: container, devices, config variables, `t_service` row (explicit confirmation in the UI); if the integration was the selected TTS provider (B.21), the `TTS_ACTIVE_PROVIDER` variable is cleared (back to Gladys Plus) |
...
| `onTtsSynthesize(cb)` | `(text, language) => Promise<string>` — "TTS provider" type (B.21): synthesize and resolve the audio data-URI (`audio/mpeg;base64,...`, ≤ 5 MB decoded); it goes into `data.audio` (30 s ack delay) |
