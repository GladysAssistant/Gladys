...
- **Discovery screen**: `GET /api/v1/external_integration/:selector/discovered_device` (the supervisor's in-memory list, with the "already created" flag) and `POST .../scan` (sends `SCAN_REQUEST` to the integration). Device creation then goes through the existing `POST /api/v1/device`, as for internal integrations.
...
| `type` | string | yes | `"device"` (only value in v1) |
...
Routes `/api/v1/external_integration`, standard Gladys user auth; **admin** required for anything that modifies, and for everything that concerns the administration of an integration (store, configuration, supervision, logs). A **non-admin** user only has access to what they need to link **their own** account on a communication integration (B.15): the list and the detail, in their **reduced view** (`{ id, name, selector, status, store_slug, manifest }` — no `docker_image`, no `containers`, no `webhooks`: the webhook URLs embed the Gladys Plus Open API key), plus the per-user contact routes.
...
| `GET .../:selector` | → detail (same fields, + the main container's `"started_at"`, + `"connection_status": { "connected", "message" }` (C.3), + `"containers": [ { "name", "status", "started_at", "ports": [{ "container_port", "host_port", "label" }] } ]` for multi-container ones — the frontend derives the "Open" links from it); **non-admin**: the reduced view |
...
| `publishDiscoveredDevices(devices)` | `POST /discovered_device` (complete list, replaces the previous one) |
