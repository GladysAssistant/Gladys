...
- `ports`: max 3 entries `{ container_port, protocol ("tcp" default), label (multi-language, en required), browsable (default true) }` — the **host** port is chosen by Gladys (free, persisted), never declared (C.7); the `label` names the "Open" link in the UI; `browsable: false` for ports that do not serve a web UI (e.g. a WebSocket endpoint for devices, the OCPP case): the supervision screen shows the assigned host port without the "Open" link;
