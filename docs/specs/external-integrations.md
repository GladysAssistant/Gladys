...
> **Living specification — source of truth.** This document specifies the behavior of external integrations and the contracts binding the monorepo to the ecosystem repos (section C: manifest, host API, WS protocol, store formats). Phase 1 is implemented; sections B.15–B.17 are phase 2 designs (their status is tracked by the workstream that ships them); B.18 and B.19 are reserved for the weather and AI provider types specified in parallel PRs; B.20 (TTS provider type) is implemented. **Rule: any PR that changes an external-integration behavior or contract modifies this file in the same diff** — spec first, code second. Field feedback (pilot ports, forum) is captured here before being coded.
...
| **2** | Mediated network discovery (passive listening **and active scan** broadcast/mDNS/SSDP by the core — full design in B.16), inbound webhooks via Gladys Plus (generic gateway → integration relay, full design in B.17), advanced config widgets, device-scoped actions (`scope: "device"`, button on the device card — in v1 the `source: "devices"` select covers the need, see C.1), integration types other than "Devices" — first the "communication" type to move Telegram & co out of the core (full design in B.15), then weather…. | An integration detects its hardware without manual config; a messaging channel installs from the store. |
...
- **Published ports**: each declared port is published on a **host port chosen by Gladys** (a free port, allocated at first start then **persisted** — stable across recreations), never by the manifest: no collision possible between integrations or with a host service. The frontend shows an "Open <label>" link in the supervision block (Frigate's UI is one click away); the integration reads the assigned port via `GET /container`. Publishing a port = exposing a third-party interface on the LAN: stated on the install screen (B.14.8).
...
- compares `index.version` vs `t_service.version` (linked by `store_slug`) → "update available" flag, the update is an explicit admin gesture (no auto-update in v1);
...
- TTS provider (B.20): manifest `type: "tts"` accepted (`messaging`/`contact_schema` still rejected outside communication), `tts.synthesize` relay (audio in `command-result.data`, **30 s** timeout honored, curated content types — own-key lookup, inherited object properties rejected —, 5 MB decoded bound, 2000-character text cap, malformed audio → throw), `gladys.tts` dispatch (variable set → provider proxy with defense-in-depth payload validation; absent → Gladys Plus path untouched including the 403/429 upsell paths; uninstalled/stopped provider → throw, no silent fallback), token audio route (200 with the curated content type, 404 unknown/expired, RAM cache purged lazily), `GET`/`POST /api/v1/tts/provider` (list with display names, `422` on an unavailable provider, admin-only write, non-admin read), uninstall of the selected provider clears `TTS_ACTIVE_PROVIDER`.
...
### B.15 "Communication" type: moving Telegram & co out of the core (design, phase 2)
...
### B.16 Mediated network discovery (design, phase 2)
...
### B.17 Inbound webhooks via Gladys Plus (design, phase 2)
...
### B.20 "TTS provider" type (implemented)

Goal: that text-to-speech stops being hardwired to Gladys Plus — a self-hosted engine (Piper-like), a cloud TTS API, or any community engine becomes an ordinary external integration of **type `tts`**, selectable as *the* voice of the instance (community request, topic 10422).

**State before this refactor (what motivated it)** — two consumers, both of which called the Gladys Plus gateway **by name**, and both needing a **URL that a player fetches itself**:
- the `MUSIC.PLAY_NOTIFICATION` scene action (`scene.actions.js`): text → `gateway.getTTSApiUrl({ text })` → signed public URL → `device.setValue(device, playNotificationFeature, url, { volume })` — the speaker (Sonos `PlayNotification({ trackUri })`, Google Cast, AirPlay) downloads the URL on its own;
- the voice assistant reply (`gateway.processVoiceMessage.js`): the answer text → same call → `ttsUrl` played by the browser (`Audio.src = ttsUrl`, absolute URL, no base prefixing).

Both consumers now go through `gladys.tts.getSpeechUrl` (below); `gateway.getTTSApiUrl` remains as the Gladys Plus transport behind the dispatcher (and behind the untouched `POST /api/v1/gateway/tts` endpoint).

**1. Core refactor (the `forwardToChannels` pattern of B.15, transposed)**: a new core manager **`gladys.tts`** (`server/lib/tts/`) decouples consumers from providers.
- **Provider interface**: any stateManager service exposing **`tts.synthesize({ text, language })`** is a TTS provider — the core knows no provider by name; an internal service could implement it tomorrow the same way. **Gladys Plus stays the built-in provider** under the reserved id **`gladys-plus`** (dispatch to the untouched `gateway.getTTSApiUrl`, signed public URL — keeps working through remote access).
- **Active provider**: system variable **`TTS_ACTIVE_PROVIDER`** (default and fallback value `gladys-plus`; otherwise a service selector). One active provider for the whole instance — the TTS voice is an instance-level choice, like the timezone. Admin UI: a "Voice (TTS provider)" card on the **"Artificial Intelligence" integration page** (`front/src/routes/integration/all/openai/`, next to the other voice/chat features rather than in the generic System settings), fed by `GET /api/v1/tts/provider` (authenticated, non-admin allowed: the frontend also uses it to stop showing the Gladys Plus upsell on the scene action when another provider is active; each provider carries a display `name` — the integration's manifest name — so the select never shows a raw selector) and saved via `POST /api/v1/tts/provider { provider }` (admin, `422` if the value is neither `gladys-plus` nor a service currently exposing `tts.synthesize`).
- **Consumers** call **`gladys.tts.getSpeechUrl({ text, language })`** → `{ url }` (`language` best effort: the user's for the voice assistant, absent in scenes). `gladys-plus` → the gateway's signed URL, unchanged behavior including the 403 (no Plus) and 429 paths that feed the existing upsell. Integration provider → the proxy service's `tts.synthesize` (WS relay below) returns the audio bytes; the core keeps them **in RAM** (10 min TTL, 32-byte random token — the discovered-devices doctrine: no table, nothing on disk) and returns `http://<LAN IPv4 of the host>:<SERVER_PORT>/api/v1/tts/audio/<token>.<ext>` (Docker bridge interfaces excluded — a speaker can never fetch there —, then RFC1918 IPv4 of a physical NIC preferred over VPN tunnels, falling back to any non-internal IPv4: best effort on multi-homed hosts).
- **`GET /api/v1/tts/audio/:token`** is **unauthenticated by design**: a Sonos cannot send a JWT — the high-entropy token is the secret, exactly the trust model of the signed Gladys Plus URL (public URL, capability in the path), scoped to a single short-lived audio. Unknown or expired token → `404`.
- **Accepted limitation, documented**: with an integration provider the URL is a **LAN** URL — LAN speakers and LAN browsers work; a browser using remote access through Gladys Plus cannot reach it (choose the `gladys-plus` provider for that use). No relaying of local audio through the gateway in v1.
- A configured provider that is stopped or disconnected → `getSpeechUrl` **throws** (surfaced in the scene error and the voice-assistant error path, like a failing `setValue`): a silent fallback to another voice would be a surprise, not a service. **Uninstalling the selected provider clears `TTS_ACTIVE_PROVIDER`** (back to the Gladys Plus default, the `AI_PROVIDER` pattern of B.19): that is not a mid-flight voice swap — it restores the default when the choice no longer exists, instead of leaving a dangling selector that fails every announcement until an admin notices.

**2. On the integration side** — the thinnest possible contract:
- **Manifest**: `type: "tts"`, nothing else specific — voice, model, speed, per-language voices… all belong to the integration's own `config_schema` (generated form), where the choices are engine-specific by nature. Same screens as a communication integration: **Configuration only** (no Devices/Discovery), catalog category `tts`; nothing is exposed to non-admins (`404`, indistinguishable from an unknown selector — a TTS provider has no per-user block).
- **WS command** `external-integration.tts.synthesize` `{ "message_id", "text", "language" }` → `command-result` with `data: { "audio": "<content_type>;base64,..." }`. Ack within **30 s** (local synthesis on a Pi is slow — a documented exception to the 5 s rule, alongside `camera.get-image` and `action.run`). Accepted content types: `audio/mpeg`, `audio/wav`, `audio/ogg`, `audio/aac` (what Sonos/Cast/browsers actually play; the URL extension is derived from it); decoded audio ≤ **5 MB**; `text` capped at **2000 characters** by the core before relay (truncation, not an error: a long AI answer must still speak its beginning rather than fail). Malformed/oversized audio → the command fails like a timeout (`ExternalIntegrationUnavailableError`).
- **SDK**: `onTtsSynthesize(cb)` — `(text, language) => Promise<string>` resolving the audio data-URI (goes into `data.audio`, C.8).

**3. Risk assessment**: nothing beyond the standard sandbox — the command is outbound-only (the integration never pushes audio uninvited), the core validates type and size before serving a byte, the audio route exposes exactly one synthesized clip per token and expires. The spoken **text** does transit through third-party code: same trust level as a `message.send` on a communication channel, covered by the existing install warning.
...
| `type` | string | yes | `"device"`, `"communication"` (B.15) or `"tts"` (B.20) |
...
No `permissions` field in v1: outbound network access is open and the install screen says so — we do not specify what we cannot enforce (see B.14). The field may appear in a future `manifest_version` when a real restriction exists.
...
**`source` — selects with dynamic options**: a `select`/`multi_select` field can replace its static `options` with `"source"`, a **reserved enum defined by the core** — never a URL or an expression: nothing arbitrary enters the rendering, extensible without breaking the schema. V1: a single value, **`"devices"`** — the frontend populates the options with the **integration's already-created devices** (label = device name, value = `external_id`), fetched via the standard endpoint for a service's devices and therefore filtered by the integration's `t_service` (zero leakage between integrations). It is the answer to the "act on a specific device" need without making the user copy an identifier: in an action (below), a `source: "devices"` select passes the chosen `external_id` to the integration like any field value. Manifest validation: `source` outside the enum, or `source` and `options` present together → rejection.

**`oauth2` type** (cloud services with browser authorization — the Netatmo case, verified in `netatmo.retrieveTokens.js`: it is the frontend that carries the flow). Rendering: a "Connect" button (+ the connection state, see `connection_status` C.3). Flow, entirely relayed — **the Gladys server knows no provider**:
1. click on "Connect" → `POST .../:selector/oauth/authorize_url { key, redirect_uri }` (C.5), relayed to the integration over WS (`oauth.get-authorize-url`, C.4) — **it** builds the URL (client_id from its config, scopes, anti-CSRF `state` it generates and retains);
2. the frontend opens the URL (popup), the user consents at the provider, which redirects to the frontend page's **generic callback route** (`.../oauth-callback?code&state`);
...
Tokens never transit through the frontend, and refreshing is the integration's business (like `netatmo.refreshingTokens` today).
...
- `ports`: max 3 entries `{ container_port, protocol ("tcp" default), label (multi-language, en required) }` — the **host** port is chosen by Gladys (free, persisted), never declared (C.7); the `label` names the "Open" link in the UI;
...
**`POST /api/integration/v1/discovered_device`** — publishes the **complete** list of discovered devices (replaces the previous one). Device object = standard Gladys format, without `service_id` or `selector` (forced server-side):
```json
{
  "devices": [
    {
...
| core → integration | `external-integration.tts.synthesize` | `{ "message_id": "uuid", "text": "...", "language": "fr" \| null }` — "TTS provider" type (B.20): synthesize the text and respond via `command-result` with `data: { "audio": "audio/mpeg;base64,..." }` (content types `audio/mpeg` \| `audio/wav` \| `audio/ogg` \| `audio/aac`, decoded audio ≤ 5 MB, `text` already capped at 2000 characters by the core); ack expected within **30 s** (local synthesis can be slow), not 5 s |
...
| `GET .../:selector` | → detail (same fields, + the main container's `"started_at"`, + `"connection_status": { "connected", "message" }` (C.3), + `"containers": [ { "name", "status", "started_at", "ports": [{ "container_port", "host_port", "label" }] } ]` for multi-container ones — the frontend derives the "Open" links from it); **non-admin**: the reduced view on a communication integration, `404` on any other (indistinguishable from an unknown selector) |
...
| `DELETE .../:selector` *(admin)* | → `200 { "success": true }` — removes **everything**: container, devices, config variables, `t_service` row (explicit confirmation in the UI); if the integration was the selected TTS provider (B.20), the `TTS_ACTIVE_PROVIDER` variable is cleared (back to Gladys Plus) |
...
| `User` | *(not forced)* | the image chooses its user; the template sets `USER node` — forcing an arbitrary uid would break legitimate images |
...
| Volumes | each `volumes[]` entry → bind `<basePath>/external-integrations/<selector>/containers/<name><path>` — host path **derived by the supervisor**, never provided by the manifest; the main container sees them under its `/data/containers/<name>/...` |
...
| `publishDiscoveredDevices(devices)` | `POST /discovered_device` (complete list, replaces the previous one — max 200 devices, 5 MB body, see C.3) |
...
| `onTtsSynthesize(cb)` | `(text, language) => Promise<string>` — "TTS provider" type (B.20): synthesize and resolve the audio data-URI (`audio/mpeg;base64,...`, ≤ 5 MB decoded); it goes into `data.audio` (30 s ack delay) |
