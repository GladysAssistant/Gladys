...
- `ports`: max 3 entries `{ container_port, protocol ("tcp" default), label (multi-language, en required), name (optional) }` — the **host** port is chosen by Gladys (free, persisted), never declared (C.7); the `label` names the "Open" link in the UI; the optional `name` (`[a-z0-9_]{2,20}`, **unique across the whole manifest**) makes the assigned host port referenceable by the `{{port:<name>}}` placeholder of the section texts (see above);
...
```json
{
  "containers": [
    ...
    { "name": "frigate", "status": "stopped", "desired": "stopped", "started_at": null,
      "ports": [{ "container_port": 5000, "host_port": 42115, "name": "frigate_ui" }],
...
| `GET .../:selector` | → detail (same fields, + the main container's `"started_at"`, + `"connection_status": { "connected", "message" }` (C.3), + `"containers": [ { "name", "status", "started_at", "ports": [{ "container_port", "host_port", "label", "name" }] } ]` for multi-container ones — the frontend derives the "Open" links and the `{{port:<name>}}` placeholder values (C.1) from it); **non-admin**: the reduced view on a communication integration, `404` on any other (indistinguishable from an unknown selector) |
