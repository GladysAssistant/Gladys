...
| core → integration | `external-integration.weather.get-image` | `{ "message_id": "uuid", "key": "vigilance-map" }` — the core asks for a provider image declared in the pivot's `images` (B.18 point 6); respond via `command-result` with `data: { "image": "<raw base64, no data-URI prefix>" }` — decoded bytes ≤ 500 KB, PNG or JPEG only (magic numbers checked by the core); ack within **15 s**; the core caches the validated image 10 min |
