...
> **Spécification vivante — source de vérité.** Ce document spécifie le comportement des intégrations externes et les contrats qui lient le monorepo aux repos de l'écosystème (section C : manifeste, API-hôte, protocole WS, formats du store). La phase 1 est implémentée ; les sections B.15–B.17 sont des designs de phase 2 non encore implémentés. **Règle : toute PR qui change un comportement ou un contrat des intégrations externes modifie ce fichier dans le même diff** — la spec d'abord, le code ensuite. Les retours terrain (portages pilotes, forum) s'instruisent ici avant d'être codés.
...
|---|---|---|
| **1** *(livrée)* | API-hôte + WS, superviseur, auth, API admin, **store décentralisé** (indexeur + catalogue + install 1 clic + mises à jour), front au niveau des intégrations internes : entrée dans le catalogue (badge « externe ») + page générique à 3 écrans Appareils / Découverte / Configuration (formulaire généré depuis le `config_schema`), SDK Node (repo dédié), template/PoC (repo dédié), **documentation publique sur le site** (interne vs externe + guide développeur). Install « dev » par image Docker conservée. | N'importe quel dev tague son repo → son intégration apparaît dans le catalogue de toutes les Gladys sans aucune approbation → un utilisateur l'installe en un clic, ses appareils découverts sont créés depuis l'UI, actionnables, configurables via formulaire généré ; l'intégration survit à un kill (redémarrage auto), passe « En panne » avec logs après échecs répétés. |
| **2** | Découverte réseau médiée (écoute passive **et scan actif** broadcast/mDNS/SSDP par le core — design complet en B.16), webhooks entrants via Gladys Plus (relais générique gateway → intégration, design complet en B.17), widgets de config avancés, actions à portée appareil (`scope: "device"`, bouton sur la fiche de l'appareil — en v1 le select `source: "devices"` couvre le besoin, cf. C.1), autres types d'intégrations que « Appareils » — en premier lieu le type « communication » pour sortir Telegram & co du core (design complet en B.15), puis météo…. | Une intégration détecte son matériel sans config manuelle ; un canal de messagerie s'installe depuis le store. |
...
- **Garantie : l'accès unicast au LAN fonctionne.** Joindre un appareil en `192.168.x` (TCP ou UDP — protocole local Tuya port 6668, API locale Shelly, MQTT local…) traverse le NAT du bridge comme n'importe quelle connexion sortante : Gladys ne filtre **rien** en egress, ni internet ni LAN. C'est une **exigence contractuelle** pour les intégrations locales, vérifiée dans le parcours e2e (section Vérification). Si ça échoue chez un utilisateur, la cause est extérieure au framework — cas classique : un pare-feu hôte strict qui DROP le FORWARD bridge→interface LAN (symptôme : cloud OK, LAN KO) — à couvrir dans la section dépannage de la doc (B.12).
- **Limite bridge assumée : pas de broadcast/multicast LAN entrant.** Les paquets broadcast, mDNS ou SSDP émis sur le LAN ne traversent pas le bridge NAT — vérifié sur un cas réel : le scan local Tuya (`tuya.localScan.js`) écoute des broadcasts UDP (ports 6666/6667/7000) et ne fonctionnerait pas dans un conteneur bridge (le core s'en sort uniquement parce qu'il est en `network=host`). La limite vaut **dans les deux sens** : un broadcast émis depuis le conteneur ne traverse pas non plus le NAT vers le LAN (cas TP-Link : requête de découverte active, cf. B.16). L'**unicast** LAN sortant, lui, traverse le NAT : joindre une IP connue (obtenue d'un cloud, saisie en config) ou une API cloud fonctionne. La **découverte médiée** (le core, en host, capture et relaie — design complet en B.16) est l'objet de la phase 2 — à documenter honnêtement d'ici là (B.12).
...
| Écran | Route | Contenu |
|---|---|---|
| **Appareils** | `/dashboard/integration/device/external/:selector` | Les appareils déjà créés de l'intégration (mêmes cartes/édition que les device-pages internes, filtrées par le `t_service` de l'intégration) ; **macaron de transport** par appareil quand le param `GLADYS_TRANSPORT` est renseigné (`local` / `cloud` / `injoignable`, tooltip traduit) — si `GLADYS_TRANSPORT_DEGRADED`, le macaron **garde sa couleur de transport** avec un **point orange** superposé et le tooltip affiche le `message` (l'appareil fonctionne, mais pas en mode nominal — cf. C.3) — + **résumé global en tête** (« 12 local · 3 cloud · 1 injoignable · 1 dégradé »), temps réel via `device-transport-updated` |
...
2. **Doc développeur : « Développer une intégration externe »** — section dédiée en deux volets :
   ...
   - *Référence* : le manifeste champ par champ avec ses règles de validation (C.1, dont cover 800×534 et bornes name/description), l'API-hôte REST (C.2–C.3), le protocole WebSocket (C.4), l'API du SDK JS (C.8), le contrat du conteneur (C.7 : rootfs read-only, `/data` seul volume, limites de ressources, variables d'environnement), les **sous-conteneurs** (déclaration `containers` = autorisation, cycle de vie via l'API `/container`, réseau privé, volumes confinés, ports publiés, classes matérielles dont le Coral — demandées par le manifeste, accordées par l'utilisateur, s'adapter via `GET /container` et `hardware-updated` —, pattern « générer la config dans `/data` puis start/restart » — avec l'exemple complet Frigate + Mosquitto), le **statut de transport par appareil** (`GLADYS_TRANSPORT`, dont l'état dégradé `GLADYS_TRANSPORT_DEGRADED`/`_MESSAGE`) et la préférence locale standard (`transports`/`GLADYS_PREFER_LOCAL`), le **flow OAuth2** des services cloud (champ `oauth2`, handlers `onOAuthAuthorizeUrl`/`onOAuthCallback`) et le **statut de connexion applicatif** (`connection_status`), les bonnes pratiques réseau (l'**unicast LAN et le cloud fonctionnent, garanti** — seul le broadcast/mDNS entrant ne passe pas en bridge, cf. B.2/B.16 ; dépannage : pare-feu hôte qui DROP le FORWARD bridge→LAN = « cloud OK, LAN KO » ; ne publier que les états qui changent, cf. rate-limit C.3), l'exigence multi-arch, le cycle de publication/mise à jour (bumper `version`), et le diagnostic en self-service via `rejected.json` (C.6).
...
- Retours terrain : **upsert des `params`** sur re-publication d'un device déjà créé (params modifiés → mis à jour en DB, name/features intouchés, aucun écho `device-updated`), **actions** (`fields` validés → `422`, timeout **par action** respecté au lieu des 5 s, `404` sur `key` non déclarée, message multi-langue), **`poll` no-op silencieux** quand l'intégration est `STOPPED`/`ERROR` (zéro log, `setValue` throw toujours), nouveaux types `multi_select`/`display: "radio"` validés, `source: "devices"` (options peuplées avec les seuls devices du `t_service` de l'intégration ; manifeste rejeté si `source` inconnue ou `source` + `options` ensemble), `section` (purement présentationnel : `required`/`default`/`placeholder` présents ou `url` non-https → manifeste rejeté ; aucune écriture en `t_variable`, absent du payload accepté par `POST .../config`), avertissement double instance (même image sans tag ou même `name`), `started_at` exposé dans le détail.
...
- Transport : param `GLADYS_TRANSPORT` validé (`local|cloud|unreachable`, clé `GLADYS_*` non définie → `400`), `POST /device/transport` (batch, param mis à jour, push WS `device-transport-updated`, external_id inconnu ignoré ; état dégradé : params `GLADYS_TRANSPORT_DEGRADED`/`GLADYS_TRANSPORT_MESSAGE` posés puis **effacés** par une entrée sans `degraded`, `message` sans `en` → `400`), interrupteur `GLADYS_PREFER_LOCAL` (rendu si `transports: ["local","cloud"]`, stocké, poussé via `config-updated`, refusé en écriture sur le `POST /config` intégration → `400`).
...
Objectif : que Telegram, Nextcloud Talk, CallMeBot, Free Mobile — et demain Matrix, Signal, WhatsApp… — soient des intégrations externes ordinaires, et que le core cesse de les connaître par leur nom. Ce design est **phase 2** ; il est écrit dès maintenant pour que les contrats v1 n'aient rien à casser.

Le type couvre **deux familles** qu'il ne faut pas confondre (retour terrain du portage Free Mobile : présupposer la bidirectionnalité bloquait le cas) : les **canaux de chat** bidirectionnels (Telegram — entrant + sortant, autorité de l'utilisateur) et les **canaux de notification** en envoi seul (Free Mobile, CallMeBot — aucun entrant possible). Le manifeste le déclare : `messaging: { "receive": true|false }` (l'envoi est toujours présent) ; tout le reste — liaison, endpoints, niveau d'avertissement — en découle.

**État des lieux (vérifié dans le code)** — trois chemins, d'asymétrie croissante :
...
- **Notification pure : encore un nom en dur, ailleurs.** Free Mobile n'est même pas un canal de chat : identifiants au niveau du service (un seul compte par instance), aucun entrant, un unique consommateur — l'action de scène `ACTIONS.SMS.SEND` qui hardcode `getService('free-mobile')` (`scene.actions.js:638`). L'action générique `ACTIONS.MESSAGE.SEND`, elle, passe déjà par `message.sendToUser(action.user, …)` : la boucle du refactor ci-dessous couvre les scènes **gratuitement**.

**1. Refactor core (préalable, bénéfique même sans intégrations externes)** : remplacer les blocs codés en dur par une boucle générique — pour chaque service du stateManager exposant `message.sendToUser(user, message)`, l'appeler ; chaque service résout lui-même son identité par user et no-op si l'utilisateur n'est pas lié. Telegram/nextcloud-talk/callmebot implémentent cette interface (leur logique actuelle déménage du core vers leur service), la liste `TOOL_CALL_EXTERNAL_SOURCES` de `message.reply.js` devient « tout service exposant `message.sendToUser` », et le core ne connaît plus aucun nom. L'action de scène `SMS.SEND` hardcodée devient redondante avec `MESSAGE.SEND` dès qu'un canal Free Mobile par utilisateur existe — dépréciée en phase 3 avec le service core (au passage, le portage lève une limite actuelle : un compte Free Mobile **par utilisateur** au lieu d'un par instance — changement de modèle visible, à documenter pour les utilisateurs existants).

**2. Côté intégration externe** — extension naturelle des contrats v1 :
- **Manifeste** : `type: "communication"` (le champ `type` de C.1 est prévu pour) + `messaging: { "receive": true|false }` ; si `receive: false`, un **`contact_schema`** (même format plat que le `config_schema`, C.1 — ex. Free Mobile : `username` + `access_token` en `secret`) décrit l'identité **par utilisateur** du canal. Pas d'écrans Appareils/Découverte — la page générique n'affiche que Configuration (elle branche déjà par type), enrichie d'un bloc « **Mon compte** » quand un `contact_schema` est déclaré : chaque utilisateur (pas seulement l'admin) y saisit **ses** valeurs, stockées en `t_variable` scoppée `(service_id, user_id)`.
- **Proxy-service** : expose `message.send(contactId, message)` (chemin réponse, déjà générique — n'existe que si `receive: true`) et `message.sendToUser(user, message)` (boucle du refactor ci-dessus, résolution d'identité par le superviseur) ; relais par WS `external-integration.message.send { message_id, contact, message: { text, file } }` avec l'ack `command-result` standard (la convention `<domaine>.<action>` de C.4 était prévue pour). **`text` est du Markdown** : le cerveau et l'IA en produisent, et le core ne peut pas le convertir à la place du canal — il ne connaît ni le sous-ensemble HTML de Telegram, ni le Markdown natif de Discord, ni le fait qu'un canal SMS n'affiche rien. C'est donc à chaque intégration de communication de rendre le Markdown dans le format de son canal (cf. C.4). Le champ `contact` porte l'identité résolue : `{ "id": "<contact_id>" }` pour un canal lié par code (`receive: true`), ou les **valeurs du `contact_schema` de l'utilisateur cible** pour un canal en envoi seul (ex. `{ "username": "...", "access_token": "..." }`) — utilisateur sans identité configurée → no-op silencieux, exactement la sémantique de la boucle `sendToUser`.
- **Entrant (`receive: true` uniquement)** : `POST /api/integration/v1/message` `{ "contact_id": "...", "text": "...", "created_at"? }` — le superviseur résout `contact_id → user` via la table de liaison, puis émet `EVENTS.MESSAGE.NEW` avec `source = <selector>` : cerveau, réponse et historique fonctionnent tels quels. Contact inconnu → `404` (l'intégration peut alors répondre dans le canal « compte non lié, code requis ») ; `403` si le manifeste déclare `receive: false` (un canal de notification ne parle jamais au cerveau).
- **Identité utilisateur↔canal, deux modes selon `receive`** :
  - `receive: true` — **liaison par code dans le canal** (généralisation du deep-link Telegram existant, `message.getCustomLink.js`) : l'utilisateur clique « Lier mon compte » sur la page de l'intégration → Gladys génère un code court en cache (TTL 15 min, lié au `user_id`) → l'utilisateur envoie ce code au bot dans le canal externe → l'intégration appelle `POST /api/integration/v1/contact/link { code, contact_id, contact_name? }` → le superviseur valide et persiste la liaison → `200 { user: { selector, first_name, language } }`. Le passage par le canal est la **preuve de contrôle du compte externe** — indispensable vu l'autorité conférée (point 3). `GET /contact` liste les contacts liés ; révocation par l'utilisateur depuis la page de l'intégration (chaque user voit et délie **son** compte).
  - `receive: false` — **saisie dans l'UI de Gladys** : pas d'entrant, donc la liaison par code est impossible par construction — et inutile (aucune autorité à protéger). L'identité vient du bloc « Mon compte » (`contact_schema`, ci-dessus) ; la révocation = effacer ses valeurs.
  - Stockage commun : `t_variable` scoppée `(service_id, user_id)` (le pattern nextcloud-talk existant — aucune migration).

**3. Risque spécifique, à traiter à la hauteur du danger — asymétrique par construction** : un message entrant a l'**autorité de l'utilisateur lié** (déclencher des scènes, ouvrir des portes via le cerveau…). La liaison par code est donc le consentement critique : code à usage unique, TTL court, généré seulement par l'utilisateur depuis l'UI, révocable ; et l'écran d'installation avertit à **deux niveaux selon `messaging.receive`** — `true` : « cette intégration pourra envoyer **et recevoir** des messages en votre nom » (classe haute, le modèle de confiance du bot Telegram actuel — mais avec du code non audité, à écrire noir sur blanc dans la doc utilisateur B.12) ; `false` : « cette intégration pourra vous envoyer des messages » (classe basse — pas d'entrant, donc **jamais** l'autorité de l'utilisateur, le `403` du superviseur le garantit côté serveur, pas seulement côté manifeste).
...
Problème (établi en B.2) : les conteneurs bridge ne reçoivent **jamais** les broadcasts, mDNS ou SSDP du LAN — seul le core, en `network=host`, les voit. Et l'inverse est vrai aussi : un broadcast **émis** depuis un conteneur bridge ne traverse pas le NAT vers le LAN. Deux cas de validation concrets, symétriques : le scan local **Tuya** (`tuya.localScan.js`) est **passif** — écouter des broadcasts UDP sur les ports 6666/6667/7000 puis parser/déchiffrer avec la lib tuyapi ; le scan **TP-Link** (protocole Kasa, remonté par le portage terrain de l'intégration) est **actif** — émettre une requête chiffrée en broadcast UDP (port 9999, 20002 pour les appareils récents) et collecter les réponses, qui reviennent en **unicast vers l'émetteur** (donc vers celui qui a émis : seul le core peut jouer ce rôle). Principe du design : **le core capture et émet (position réseau), l'intégration interprète et forge (connaissance du protocole)** — le core ne parse ni ne fabrique jamais un payload.
...
Types v1 du champ (extensibles par version de schéma) : `udp-broadcast` (écoute passive sur les ports déclarés, max 5 ports), `udp-active-broadcast` (requête/réponse : émission d'un payload fourni par l'intégration en broadcast sur les ports déclarés, max 5 ports, puis collecte des réponses unicast — le cas TP-Link), `mdns` (browse d'un type de service déclaré), `ssdp` (M-SEARCH sur un `st` déclaré). Affiché sur l'écran d'installation (« cette intégration pourra écouter les annonces réseau UDP ports 6666–6667 », « pourra émettre une requête de découverte en broadcast UDP port 9999 ») ; jamais de capture arbitraire (pas de pcap, pas de port non déclaré).
...
- **Scan actif** : même endpoint, `{ "type": "udp-active-broadcast", "port": 9999, "payload_base64": "...", "timeout_seconds": 5 }` — le core émet le payload en broadcast sur le port (limited broadcast `255.255.255.255` + broadcast dirigé de chaque interface non-Docker), puis collecte les réponses unicast pendant le timeout ; retour = même format brut que `udp-broadcast`. Garde-fous propres à l'émission (le core envoie un paquet forgé par un tiers, la primitive doit rester inintéressante à détourner) : **broadcast uniquement** (jamais d'unicast vers une cible choisie — pas de sweep du LAN par procuration), port ∈ ceux déclarés au manifeste, `payload_base64` ≤ 512 octets décodés, **1 scan / 10 s par intégration** (`429` sinon) — ni ciblage, ni scan de ports, ni volume.
- L'intégration parse elle-même (Tuya : `MessageParser` sur les `payload_base64`, exactement le code de `localScan` actuel ; TP-Link : forge la requête Kasa chiffrée, déchiffre les réponses — la crypto reste dans le conteneur), puis joint les appareils en **unicast** (qui traverse le NAT, déjà possible en v1) et publie ses `discovered_device`.
...
**Ce que la v1 doit anticiper (et anticipe déjà)** : champ de manifeste additif (un manifeste avec `network_discovery` est simplement refusé par une Gladys trop vieille via `gladys_version`), API-hôte versionnée, écran d'installation déjà structuré en « demandes » approuvées (conteneurs, matériel — les captures s'y rangeront). En attendant : Tuya externe = cloud + poll local unicast (IPs obtenues du cloud), sans scan LAN ; TP-Link externe = saisie manuelle des IPs (rendue vivable par l'upsert de `params` et les actions à select `source: "devices"`, C.1/C.3) — documenté honnêtement (B.2, B.12). Deux portages pilotes sur deux ont buté sur la découverte (Tuya en passif, TP-Link en actif) : B.16 est le **premier chantier de phase 2**, et le scan actif seul (un socket `dgram` qui émet puis collecte) est détachable si on veut le sortir vite.

### B.17 Webhooks entrants via Gladys Plus (design, phase 2)

Problème : des services tiers poussent leurs événements par webhook (Netatmo : changements de consigne en ~2-3 s au lieu du poll à 2 min), mais une instance Gladys locale n'est pas joignable depuis Internet. Le core a déjà résolu ce cas **pour Netatmo** (PR #2627) via une route de la gateway Gladys Plus câblée en 2021 : `POST https://api.gladysgateway.com/v1/api/netatmo/:open_api_key` relaie le corps brut à l'instance par le WebSocket Plus (message `gladys-open-api`). Le portage Netatmo externe est bloqué dessus : le chemin gateway → conteneur d'intégration n'existe pas. On industrialise — **pour toutes les intégrations, sans que la gateway n'en connaisse aucune**.

**Déclaration dans le manifeste** — champ optionnel `webhooks` (≤ 3), affiché à l'install (« pourra recevoir des événements depuis Internet via Gladys Plus ») :
...
Deux modes, car les deux existent sur le terrain : **`fire_and_forget`** (défaut) — le tiers pousse et n'attend qu'un accusé (la classe Netatmo) ; **`sync`** — l'appelant attend la **réponse de l'intégration** (challenge/réponse à l'enregistrement type Strava/Microsoft Graph, retours applicatifs).

**Côté cloud (repo `GladysAssistant/gladys-gateway`, hors de l'écosystème listé en fin de spec — porté par le mainteneur, la spec définit le contrat)** : une route générique `GET|POST /v1/api/external-integration/:open_api_key/:selector/:webhook_key` qui relaie `{ selector, webhook_key, method, query, body brut, content_type }` (body ≤ 256 Ko) sous l'action unique `external-integration-webhook`, attend l'ack de l'instance avec un **timeout dur** (10 s) et répond ce que l'ack contient ; **timeout ou instance injoignable → `200` corps vide, toujours** — c'est le fix durable de la leçon Netatmo (5 échecs consécutifs = webhook banni par le tiers, observé en vrai ; PR #2627 note que la gateway actuelle laisse la requête échouer). La gateway relaie, point : zéro connaissance des intégrations.

**Côté instance** : une branche `external-integration-webhook` dans `gateway.handleNewMessage.js` → le superviseur vérifie selector + `webhook_key` déclaré (inconnu → ack `200` vide, silencieux — pas de fuite de validité), puis selon le mode :
- `fire_and_forget` : ack `{ status: 200 }` **immédiat**, relais asynchrone par le WS intégration — `external-integration.webhook.received` `{ "webhook_key", "method", "query", "body", "content_type" }`, sans `message_id` ni ack ; intégration déconnectée/arrêtée → perdu sans erreur (voir doctrine ci-dessous) ;
- `sync` : relais `external-integration.webhook.request` (mêmes champs + `message_id`), ack standard 5 s attendu ; le `command-result.data` de l'intégration — `{ "status": 200–499, "content_type", "body" }` (body ≤ 64 Ko) — est renvoyé tel quel à la gateway, donc au tiers ; timeout ou intégration arrêtée → `{ status: 200 }` vide.

**Doctrine « déclencheur, pas donnée »** (leçon de terrain du PR #2627, à transposer dans la doc développeur B.12) : les événements webhook arrivent dupliqués, en retard, dans le désordre (observé : un événement périmé livré une minute après son successeur), et leurs payloads sont partiels ou trompeurs. Un webhook sert à **déclencher un refresh** par l'API du fabricant, jamais à appliquer le payload comme un état — c'est aussi ce qui rend indolores les pertes en `fire_and_forget` : le poll reste la source de vérité et rattrape tout.

**Clé et UI (bloc standard, rendu par le core — patron `GLADYS_PREFER_LOCAL`)** : l'utilisateur crée sa clé Open API dans Gladys Plus (Réglages → Open API) et la colle dans un bloc « **Webhooks Gladys Plus** » de l'écran Configuration, affiché seulement si le manifeste déclare `webhooks` (Plus non lié → message explicatif à la place). Stockée sous la clé de config réservée **`GLADYS_OPEN_API_KEY`** (secret, scoppée au `service_id`, refusée en écriture sur le `POST /config` intégration comme toute clé `GLADYS_*`). Le core construit les URLs complètes. Manuel assumé : c'est déjà l'utilisateur qui manipule l'URL côté fabricant ; le provisionnement automatique (clé mintée par intégration via l'API Plus) est une amélioration ultérieure qui ne changera pas le contrat intégration.

**Côté intégration** : `GET /api/integration/v1/webhook` → `{ "available": true|false, "webhooks": [ { "key", "mode", "url" } ] }` (URLs prêtes à enregistrer chez le tiers — le pattern Netatmo : `addwebhook` à chaque connexion réussie, best-effort) ; événement `external-integration.webhook-updated` quand la disponibilité change (Plus lié/délié, clé changée). `available: false` (pas de Gladys Plus) → l'intégration dégrade en poll seul. SDK : `getWebhooks()`, `onWebhook(key, cb)` — en `fire_and_forget` le retour du callback est ignoré ; en `sync` la valeur résolue `{ status?, contentType?, body? }` part dans le `command-result`.

Sécurité, dit honnêtement : l'URL **est** le secret (payloads non authentifiés — quiconque a l'URL peut poster ; la vérification d'authenticité — signature du fournisseur — appartient à l'intégration) ; l'intégration voit la clé Open API (elle est dans l'URL qu'elle enregistre chez le tiers) — clé scoppée au relais Open API, pas au compte Plus, et révocable par l'utilisateur dans Gladys Plus ; les événements d'une intégration ne sont routés qu'à elle (selector dans l'URL + `webhook_key` déclaré) ; tailles bornées aux deux sens.
...
| `transports` | array | non | canaux supportés, sous-ensemble de `["local", "cloud"]` ; les deux présents → interrupteur standard « Privilégier la connexion locale » (voir ci-dessous) |
...
**`config_schema`** : volontairement une liste plate de champs, pas du JSON Schema complet — le rendu du formulaire reste déterministe et sans surprise (principe « UI déclarative »). Champs par entrée : `key` (unique, `[a-z0-9_]`), `type` (`string` | `number` | `boolean` | `select` | `multi_select` | `secret` | `oauth2` | `section`), `label` (multi-langue, `en` obligatoire), `description` (multi-langue, optionnel), `placeholder` (multi-langue, optionnel — texte d'exemple affiché dans le champ vide, ex. `48.85`), `required` (défaut `false`), `default`, `min`/`max` (number), `options` (select/multi_select : `[{ value, label }]`) **ou** `source` (options dynamiques fournies par le core, voir ci-dessous — mutuellement exclusif avec `options`), `display` (select : `"dropdown"` défaut ou `"radio"` — boutons radio). Rendus : `boolean` = interrupteur/case à cocher ; `multi_select` = cases à cocher, valeur stockée = array de `value`s. Les valeurs sont stockées dans `t_variable` scoppées par `service_id` ; les `secret` ne sont **jamais renvoyés au front** (cf. C.5), mais sont fournis à l'intégration.

**`section` — chapitres d'amorce** (besoin terrain du portage Netatmo : devant « Client ID », l'utilisateur doit savoir qu'il faut d'abord créer une app sur la plateforme développeur du fabricant — les intégrations core ont ce texte dans leurs pages JSX, le formulaire généré ne l'avait pas) : champ **purement présentationnel**, aucune valeur stockée (`required`/`default`/`placeholder` présents → manifeste rejeté ; jamais d'écriture en `t_variable`). Contenu : `label` (multi-langue, `en` obligatoire — le titre du chapitre), `description` (texte **brut** multi-langue, ≤ 1000 caractères par langue), `links` (optionnel, ≤ 5 entrées `[{ "url", "label" }]` — `url` en **https obligatoire**, `label` multi-langue). Rendu par le core : séparateur visuel + texte + liens ouverts dans un nouvel onglet avec le **domaine cible affiché** à côté du label (contenu tiers non modéré, même niveau de confiance que la doc re-hébergée : l'utilisateur voit où il clique). **Pas de markdown ni de HTML** — la version déclarative du besoin, zéro contenu riche interprété (principe « UI déclarative »). Le `config_schema` étant une liste ordonnée, les sections structurent naturellement les gros formulaires ; et comme les `fields` des actions réutilisent ce format, elles y sont disponibles aussi. Pour le pas-à-pas long (captures d'écran…), c'est la documentation obligatoire du repo qui reste le bon support (B.9), accessible en permanence depuis l'écran Configuration (B.8).

**`source` — selects à options dynamiques** : un champ `select`/`multi_select` peut remplacer ses `options` statiques par `"source"`, un **enum réservé défini par le core** — jamais une URL ni une expression : rien d'arbitraire n'entre dans le rendu, extensible sans casser le schéma. V1 : une seule valeur, **`"devices"`** — le front peuple les options avec les **appareils déjà créés de l'intégration** (label = nom de l'appareil, value = `external_id`), récupérés par le endpoint standard des devices d'un service et donc filtrés par le `t_service` de l'intégration (zéro fuite entre intégrations). C'est la réponse au besoin « agir sur un appareil précis » sans faire recopier un identifiant à l'utilisateur : dans une action (ci-dessous), un select `source: "devices"` transmet l'`external_id` choisi à l'intégration comme n'importe quelle valeur de champ. Validation manifeste : `source` hors enum, ou `source` et `options` présents ensemble → rejet.
...
Les actions sont **à portée intégration** : un seul lieu de rendu, l'écran Configuration — pas de boutons injectés par le manifeste sur les cartes d'appareils en v1 (exigence « UI cohérente » du cadrage : où les rendre, combien, sur quelles cartes n'ont pas de bonne réponse générique). Le besoin « agir sur un appareil précis » passe par un champ `source: "devices"` comme dans l'exemple ci-dessus ; des actions à portée appareil (`scope: "device"`, bouton sur la fiche de l'appareil) restent une piste **phase 2** si l'usage le réclame.

**`transports`** : déclare les canaux que l'intégration sait utiliser. Si `["local", "cloud"]` (les deux), Gladys rend en tête de l'écran Configuration un **interrupteur standard, traduit par le core** — même libellé pour toutes les intégrations : « Privilégier la connexion locale (LAN) quand elle est disponible » — stocké sous la clé de config réservée **`GLADYS_PREFER_LOCAL`** (boolean, défaut `true`). L'intégration la reçoit comme n'importe quelle clé (`GET /config`, événement `config-updated`) mais ne peut pas l'écrire (clé `GLADYS_*` sur le `POST /config` intégration → `400` : c'est une préférence utilisateur). La préférence est un **souhait**, pas un ordre : l'intégration l'applique quand elle peut, et reflète la réalité par appareil via `GLADYS_TRANSPORT` (C.3) — préférence standard + statut standard, zéro sémantique fabricant dans le core.
...
**Params réservés `GLADYS_*`** : les noms de `params` préfixés `GLADYS_` sont réservés aux sémantiques définies par cette spec — toute clé `GLADYS_*` non définie → `400`. Défini en v1 : **`GLADYS_TRANSPORT`** = `"local"` | `"cloud"` | `"unreachable"`, le **statut de transport effectif de l'appareil**. C'est le motif générique des intégrations à double canal (Tuya cloud+LAN, Shelly, eWeLink/Sonoff, Somfy TaHoma, caméras Netatmo, Zendure MQTT local…) : le transport peut différer d'un appareil à l'autre et changer dans le temps, et sans indication visible l'utilisateur ne peut pas diagnostiquer un appareil lent ou figé. L'intégration le renseigne à la découverte et le tient à jour (re-publication, ou l'endpoint dédié `POST /device/transport` ci-dessous) ; Gladys le rend en **macaron** sur les appareils (B.8). Purement déclaratif : **aucune sémantique de routage côté core**, la « tambouille » cloud/local reste dans le conteneur.

S'y ajoutent deux clés réservées pour l'**état dégradé** — le cas « ça marche, mais pas comme prévu », que les trois valeurs de transport ne savent pas exprimer (cas terrain : appareil détecté par le scan local, mais sessions locales refusées — clé locale rotée, autre client qui tient la connexion… — → bascule en fallback cloud ; l'utilisateur voit un macaron « cloud » parfaitement normal et rien ne l'invite à investiguer) : **`GLADYS_TRANSPORT_DEGRADED`** (`"true"`, absente sinon) et **`GLADYS_TRANSPORT_MESSAGE`** (objet multi-langue sérialisé en JSON, `en` obligatoire, ≤ 200 caractères par langue — la raison, ex. « Local détecté mais sessions refusées, bascule cloud »). Le dégradé est volontairement **orthogonal au transport** — pas une 4e valeur de l'enum : « quel canal est utilisé en ce moment » et « est-ce l'état nominal » sont deux informations, et c'est leur combinaison (« cloud + dégradé ») qui permet de diagnostiquer.
...
**`POST /api/integration/v1/device/transport`** — corps `{ "transports": [ { "device_external_id": "ext:tuya-demo:plug1", "transport": "cloud", "degraded": true, "message": { "en": "Local session refused, falling back to cloud", "fr": "Session locale refusée, bascule cloud" } } ] }` (batch ≤ 100 ; `degraded` défaut `false`, `message` optionnel et seulement pris en compte si `degraded`) → `200 { "success": true }`. Met à jour les params `GLADYS_TRANSPORT*` des devices concernés **sans re-publier la liste découverte** (le chemin léger pour les bascules à chaud : le lien cloud saute → `unreachable`, le LAN revient → `local`) — une entrée sans `degraded` **efface** les params dégradés (retour au nominal explicite, pas d'état orange fantôme) — et pousse `device-transport-updated` au front (macarons en temps réel). Valeur hors `local|cloud|unreachable`, ou `message` sans `en` → `400` ; un `device_external_id` inconnu est ignoré silencieusement.
...
| core → intégration | `external-integration.message.send` | `{ "message_id": "uuid", "contact": { ... }, "message": { "text": "...", "file": "image/jpg;base64,..." \| null } }` — réponse du cerveau ou notification à délivrer sur le canal (type « communication », B.15) ; `contact` porte l'identité résolue (cf. B.15). **`text` est du Markdown** — le cerveau et l'IA en produisent (`**27 °C**`, titres, listes, `code`) —, à l'intégration de le rendre dans le format de son canal : HTML pour Telegram, Markdown natif pour un canal qui le gère, texte brut sinon. Une intégration qui délivre `text` tel quel affiche la syntaxe à l'utilisateur |
...
Convention de nommage des commandes : **un type spécifique par action**, `external-integration.<domaine>.<action>` — pas de type générique avec champ `action`. Les commandes futures (phase 2 : `webhook.received`/`webhook.request` pour les webhooks Gladys Plus (B.17), `camera.start-streaming` pour le flux vidéo continu… selon les types d'intégrations ajoutés) suivront ce schéma, chacune avec `message_id` + ack `command-result` (sauf exception documentée, ex. `webhook.received` volontairement sans ack, cf. B.17) ; une intégration ignore silencieusement un type qu'elle ne connaît pas.
...
Messages core → front (UI temps réel, sur le WS utilisateur existant) : `external-integration.status-changed` `{ "selector", "status" }`, `external-integration.discovered-devices-updated` `{ "selector" }`, `external-integration.connection-status-updated` `{ "selector", "connected", "message" }` (cf. C.3) et `external-integration.device-transport-updated` `{ "selector", "transports": [ { "device_external_id", "transport", "degraded", "message" } ] }` (macarons, cf. C.3).
...
| `publishTransports(transports)` | batch `POST /device/transport` — `[{ external_id, transport: 'local' \| 'cloud' \| 'unreachable', degraded?, message? }]` (macarons + état dégradé, cf. C.3) ; la préférence utilisateur arrive dans `gladys.config.GLADYS_PREFER_LOCAL` |
...
Les contrats de la section C découplent cinq repos ; le `manifest.schema.json` a un **propriétaire canonique : `GladysAssistant/integration-store`** (publié sur Pages à côté de l'index), le monorepo en embarque une copie vendorée.

| Repo | Rôle | Sections |
|---|---|---|
| `GladysAssistant/Gladys` (monorepo) | Superviseur, API-hôte, WS, store côté serveur, front — **et cette spec** | B.1–B.9, B.13, C.2–C.5, C.7 |
| `GladysAssistant/integration-store` | Indexeur (GitHub Action) + `index.json` sur Pages ; schéma canonique du manifeste | B.9, C.1, C.6 |
| `GladysAssistant/integration-sdk-js` | Paquet npm `@gladysassistant/integration-sdk` — ne dépend que des contrats C, aucun import du monorepo | B.10, C.8 |
| `GladysAssistant/integration-template-js` | Template repository officiel + PoC du parcours e2e (publié dans le store comme le ferait un dev tiers) | B.11, C.1, C.7 |
| `GladysAssistant/v4-website` | Documentation publique fr + en (transposition des sections B/C — la spec reste la source de vérité) | B.12 |
