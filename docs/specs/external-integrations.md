...
### B.18 "TTS provider" type (implemented)
...
**Current state (verified in the code)** — two consumers, both calling the Gladys Plus gateway **by name**, and both needing a **URL that a player fetches itself**:
...
- A configured provider that is uninstalled, stopped or disconnected → `getSpeechUrl` **throws** (surfaced in the scene error and the voice-assistant error path, like a failing `setValue`): a silent fallback to another voice would be a surprise, not a service.
...
| `type` | string | yes | `"device"`, `"communication"` (B.15) or `"tts"` (B.18) |
...
| `onTtsSynthesize(cb)` | `(text, language) => Promise<string>` — "TTS provider" type (B.18): synthesize and resolve the audio data-URI (`audio/mpeg;base64,...`, ≤ 5 MB decoded); it goes into `data.audio` (30 s ack delay) |
