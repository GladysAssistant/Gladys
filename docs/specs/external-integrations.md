...
- `POST /api/integration/v1/network_discovery/scan` `{ "type": "udp-broadcast", "timeout_seconds": 10 }` (1–30 s, `403` if the type/ports are not declared in the manifest) → `200` with the **raw** results: `udp-broadcast` → `[ { "source_ip", "source_port", "payload_base64" } ]`; `mdns` → `[ { "name", "host", "addresses", "port", "txt" } ]`; `ssdp` → `[ { "source_ip", "source_mac", "source_port", "headers" } ]`, the raw headers per responder. `source_mac` is **optional and best-effort**: the core looks each responder IP up in its own neighbour (ARP) table — it runs `network=host`, and a device that just answered the M-SEARCH is usually resolved there — and omits the field when the kernel does not know the address (non-Linux host, unreadable `/proc/net/arp`, incomplete entry, responder behind a router). It saves the integration from asking the user to type a MAC by hand to enable Wake-on-LAN (C.3 `POST /network/wake`) on a device it has just discovered; an integration must still treat its absence as normal.
