...
- **Manifest**: `type: "tts"`, nothing else specific — voice, model, speed, per-language voices… all belong to the integration's own `config_schema` (generated form), where the choices are engine-specific by nature. Same screens as a communication integration: **Configuration only** (no Devices/Discovery), technical type `tts`; nothing is exposed to non-admins (`404`, indistinguishable from an unknown selector — a TTS provider has no per-user block).
