...
- on republication, the derived features already in DB are reinjected with their **DB identity** before the derivation, so an "Update" updates those rows instead of duplicating them, and a derived feature whose index feature disappeared from the publication is dropped with it;
