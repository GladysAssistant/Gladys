...
| **2** | Mediated network discovery (passive listening **and active scan** broadcast/mDNS/SSDP by the core — full design in B.16), inbound webhooks via Gladys Plus (generic gateway → integration relay, full design in B.17), advanced config widgets, device-scoped actions (`scope: "device"`, button on the device card — in v1 the `source: "devices"` select covers the need, see C.1), integration types other than "Devices" — first the "communication" type to move Telegram & co out of the core (full design in B.15), the "ai" type to plug community AI providers in place of Gladys Plus (B.19, shipped), then weather…. | An integration detects its hardware without manual config; a messaging channel installs from the store. |
...
- AI provider (B.19): manifest `type: "ai"` accepted (device-agnostic fields still allowed, `messaging`/`contact_schema` still rejected outside communication), `ai.chat` relay (completion in `command-result.data`, 120 s timeout honored, malformed completion — `data` without an object `choices[0].message` — → throw, disconnected integration → throw), `gateway.aiChat` routing (variable set → proxy called with `model` stripped and hints passed through; proxy without `ai` capability → error; variable absent → Gladys Plus path untouched, including model normalization), `message.create` engages the AI path with an external provider and no Gladys Plus (and still replies "Plus required" with neither), `GET`/`POST /api/v1/ai_provider` (list + selection, `400` on a non-ai integration, `404` on an unknown selector, `null` reset, admin-only write), uninstall of the selected provider clears `AI_PROVIDER`.
...
### B.19 "AI provider" type: pluggable AI backends (implemented)

Goal (community RFC, topic 10421): let external integrations provide the **AI backend** of the assistant — Claude, DeepSeek, OpenAI, Mistral, a local LLM… — instead of Gladys Plus. Gladys Plus remains the default for every connected user; switching to an external provider is an **explicit user choice**, per instance.

**The contract is provider-agnostic by construction.** Verified in the code: the whole agentic loop — MCP tool calling, intent router, system prompts, conversation history, image resizing — runs **on the Gladys instance** (`gateway.forwardMessageToAiChat`), and the only thing that leaves is `gateway.aiChat(body)`: an **OpenAI-compatible chat completion request** (`messages`, `tools`, `tool_choice`, plus the Gladys hints `purpose` and `categories`), answered by an OpenAI-compatible completion (`choices[0].message` with `content` / `tool_calls`). That format is the de facto industry standard — every major provider either speaks it natively or is a mechanical translation away (Anthropic, Gemini…). The core therefore knows **no provider**: the external integration is a thin adapter that receives the request, calls whatever API it wants with whatever SDK it wants, and returns a completion.

- **Manifest**: `type: "ai"` (C.1). No device screens — the generic page shows Configuration/Supervision/Logs only, like a communication integration; the card lands in the Communication category of the catalog, next to the native AI page, and stays **admin-only** (there is no per-user block: the provider serves the whole instance). Everything provider-specific — API key, endpoint URL, model choice, temperature… — lives in the integration's own `config_schema`: the core never validates nor understands those values. `containers` (B.2) are allowed like for any integration: an "Ollama" integration bundling a local inference server as a sub-container is an intended use case.
- **Proxy service capability `ai.chat(request)`** (the B.15 extension pattern: `device.*`, `message.*`, now `ai.*`): relay over WS `external-integration.ai.chat { message_id, request }` with the standard `command-result` ack carrying the completion in `data` (C.4). Ack deadline **120 s** (third documented exception to the 5 s rule, after `camera.get-image` and per-action timeouts): reasoning models are slow, and one request can carry a full tool-calling context. A malformed `data` (anything without an object `choices[0].message`) fails at the contract boundary with `ExternalIntegrationUnavailableError` (the standard AI failure reply, never a crash) — adapter bugs surface as an explicit invalid-response error instead of a silent empty turn.
- **Selection: one provider per instance, stored in the system variable `AI_PROVIDER`** (untouched = Gladys Plus). Admin gesture from the AI settings page (the `openai` integration page): a select listing Gladys Plus plus every installed `type: "ai"` integration (`GET /api/v1/ai_provider`, C.5). Setting it validates that the selector is an installed `ai` integration; **uninstalling the selected provider clears the variable** (silent return to the Gladys Plus default — no dangling selector).
- **Routing**: `gateway.aiChat` is the single funnel of all AI traffic (chat loop, intent router, weekly digest) — the selected provider therefore serves **all** of it, not just the chat. The `model` field is **stripped** before the relay: model ids are Gladys Plus (Scaleway) vocabulary, meaningless elsewhere — an external provider chooses its model in its own config (the frontend hides the per-message model selector when an external provider is active). The `purpose` and `categories` hints pass through: they are generic ("chat" / "router" / digest, intent categories) and let a smart adapter pick a cheap model for the router and a strong one for scene creation, exactly like the Plus gateway does.
- **No silent fallback (privacy rule)**: a stopped, broken or disconnected provider fails the request — surfaced as the standard AI failure reply — and is **never** silently rerouted to Gladys Plus. Choosing an external provider can be a privacy decision ("my conversations only go to X"): Gladys does not send them elsewhere because X is down.
- **AI without Gladys Plus**: `message.create` engages the AI path when Gladys Plus is configured **or** an external provider is selected — an instance with no Plus account gets the full assistant through its provider. Neither → the "Gladys Plus required" reply, as today. Voice remains split: STT/TTS stay on their existing channels (Gladys Plus), only the **text** intelligence goes through the provider.
- **Trust, said plainly**: the AI provider sees every chat message, the conversation history, the tool schemas and tool results (device states, sensor values), and chat images (vision requests pass the OpenAI `image_url` content through; a text-only provider errors on them) — and its completions **drive the tool-calling loop with the user's authority** (device control, scene creation). That is the highest trust class an integration can hold; the install screen of a `type: "ai"` integration says it in those terms, and the user docs (B.12) spell it out.
...
| `type` | string | yes | `"device"` \| `"communication"` (B.15) \| `"ai"` (B.19) |
...
| core → integration | `external-integration.ai.chat` | `{ "message_id": "uuid", "request": { "messages": [...], "tools": [...], "tool_choice": "auto"\|"required", "purpose": "chat"\|..., "categories": [...] } }` — an AI request routed to the integration because it is the selected AI provider (`type: "ai"`, B.19). `request` is an **OpenAI-compatible chat completion request** (the `model` field is stripped by the core: the provider picks its own model); respond via `command-result` with `data` = an **OpenAI-compatible completion** (`choices[0].message` with `content` / `tool_calls`); ack expected within **120 s** (reasoning models are slow — third exception to the 5 s rule) |
...
| `DELETE .../:selector` *(admin)* | → `200 { "success": true }` — removes **everything**: container, devices, config variables, `t_service` row (explicit confirmation in the UI); if the integration was the selected AI provider (B.19), the `AI_PROVIDER` variable is cleared (back to Gladys Plus) |

**AI provider selection (B.19)** — dedicated literal routes (outside `/external_integration`, the choice includes Gladys Plus which is not an external integration):

| Method & route | Body → Response |
|---|---|
| `GET /api/v1/ai_provider` | → `{ "selector": "ext-..." \| null, "providers": [ { "selector", "name", "status" } ] }` — `selector` null = Gladys Plus (default); `providers` = the installed `type: "ai"` integrations. Authenticated (the chat hides its model selector when an external provider is active) |
| `POST /api/v1/ai_provider` *(admin)* | `{ "selector": "ext-..." \| null }` → `200` same shape as the `GET`; `null` resets to Gladys Plus; a selector that is not an installed `type: "ai"` integration → `400` (unknown selector → `404`) |
...
| `onAiChat(cb)` | `(request) => Promise<object>` — handler of the `ai.chat` relay (`type: "ai"` integrations, B.19): receives an OpenAI-compatible chat completion request, resolves an OpenAI-compatible completion (it goes into `data`; 120 s ack delay, see C.4) |
