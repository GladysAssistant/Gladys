...
- Calendar type (B.19): manifest `type: "calendar"` accepted, `account_schema` validated like `contact_schema` (`oauth2` and `{{port:<name>}}` refused in milestone 1); host-API gate (`403` on every calendar endpoint for the other types); per-user account routes (`account_schema` validation → `422`, enable idempotent, disable destroys the requesting user's calendars only, per-user isolation: another user's account or calendar → `404`); upsert idempotence (same push twice → zero writes), selector collision (two users, two same-named calendars → `buildUniqueSelector`), field ownership on republication (`sync`/`shared`/`selector` untouched, `name`/`description`/`color` overwritten), window prune (integration-prefixed absent events deleted, manual unprefixed events preserved, event outside the window → `400`), normalization bounds (counts and caps, lengths, invalid `color` dropped, `end` < `start` → `400`, 10 000-events-per-calendar cap), rate limit → `429`, tenant isolation (A's token cannot write B's calendars, unprefixed `external_id` rejected), push on a `sync: false` calendar → `403` and events emptied on the toggle, explicit uninstall cleanup (no reliance on the FK cascade), WS pushes (`calendar.updated` to the right users, `calendar.account-updated` to the integration), and a CalDAV regression pass (its sync loops untouched by the new primitives until it migrates).
...
- **Per-user activation is the consent gesture.** The generic page shows **every user** (not only admins) a "**My calendars**" block: the `account_schema` fields (their values) and an Enable action; once enabled, the list of **their** calendars pushed by the integration, each with its **`sync`** and **`shared`** toggles, and a Disable action that **destroys their calendars** (explicit confirmation). An integration with nothing to ask per user (a public feed) simply declares no `account_schema`: enabling remains the per-user gesture — enabling = consenting to calendars appearing in your view — and a household-wide calendar is obtained by sharing it (`shared`), the existing model.
...
**3. Host API** — all endpoints below require `type: "calendar"` in the manifest (`403 FORBIDDEN` otherwise, the B.15 gate pattern), tenant isolation by the JWT as everywhere (an integration only ever sees or writes rows whose `service_id` is its own), `external_id`s prefixed `ext:<selector>:` (C.2 conventions):
...
{
  "calendar_external_id": "ext:john-gcal:primary",
  "window": { "from": "2026-08-01T00:00:00.000Z", "to": "2026-09-01T00:00:00.000Z" },
  "events": [
    {
      "external_id": "ext:john-gcal:primary:8f3a@google.com",
      "name": "Dentist",
    ...
    {
      "external_id": "ext:john-gcal:primary:holiday-20260815",
      "name": "Assomption",
...
- **Window prune** — deletions without bookkeeping: with `window: { from, to }`, events of that calendar whose `start` falls in the window, whose `external_id` carries the integration's prefix, and which are absent from the pushed list are **deleted** — a provider-side deletion propagates by simply republishing the window, idempotently. Events created by hand in Gladys (unprefixed `external_id`) are **never pruned**. Every pushed event must start within the window (`400` otherwise — keeps the replace semantics crisp). Without `window`: pure upsert, no deletion.
...
| `POST .../:selector/calendar/account` *(any user)* | `{ "config": {...} }` validated against the `account_schema` (`422` otherwise) → `200` — enables the integration for the requesting user (idempotent; also how account values are edited) + `calendar.account-updated` push |
...
**Frontend**: the catalog's **Calendar category already exists** (`front/src/config/integrations/calendars.json` — today CalDAV alone) and the `type: "calendar"` manifest maps external integrations into it, with the external badge and status like everywhere; `EXTERNAL_CATEGORIES` (`front/src/routes/integration/index.js`) gains `'calendar'`, and `ExternalIntegrationPage`'s `hasDeviceScreens` already branches by type (calendar joins communication and weather in the no-device-screens set). The generic page gains the "My calendars" block (point 2); the calendar view gains the `calendar.updated` listener (point 1) — no other view change: pushed calendars render exactly like CalDAV ones.
