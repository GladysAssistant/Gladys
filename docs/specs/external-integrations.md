...
- **Current, optional**: `apparent_temperature` (feels-like — OW `feels_like`, MF `windchill`, HA), `humidity`, `pressure`, `dew_point`, `wind_speed`, `wind_direction` (degrees), `wind_gust`, `visibility`, `cloud_cover`, `uv_index`, `sunrise`, `sunset` (dates). Percentages (`humidity`, `cloud_cover`, `precipitation_probability` below) are **0–100** — the widget appends `%` as-is — and are **clamped to [0, 100]** by `normalizeWeather` (never fractional 0–1 scales).
