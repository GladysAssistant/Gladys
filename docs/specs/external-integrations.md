...
- **Devices too**: a weather integration keeps the **full device surface** (Devices/Discovery screens, discovered devices, states) — nothing is subtracted. This is the generic answer to provider-specific alerting scenes: Météo France publishes e.g. a "vigilance level" sensor device, and the existing device-feature scene triggers fire on level changes with **zero core code** — no weather-specific scene trigger to invent.

**3. The pivot weather format** (what `weather.get` resolves with, whoever the provider is). Required: `temperature` (number), `weather` (condition, one of `clear` | `cloud` | `fog` | `drizzle` | `rain` | `sleet` | `snow` | `thunderstorm` | `wind` | `night` | `unknown` — anything else is coerced to `unknown`), `datetime` (date), `units` (stamped by the core, see above). Optional current fields: `humidity`, `pressure`, `wind_speed`, `wind_direction`, `uv_index`, `sunrise`, `sunset`. Optional `hours` (≤ 24 entries: `temperature`, `weather`, `datetime` required; `humidity`, `pressure`, `wind_speed`, `wind_direction`, `precipitation_probability` optional) and `days` (≤ 8 entries: `temperature_min`, `temperature_max`, `datetime` required; `weather`, `precipitation_probability` optional). Optional `alerts` (≤ 10 entries — the Météo France vigilance case, but generic: CAP-style severities): `severity` (`minor` | `moderate` | `severe` | `extreme`) and `event` (≤ 100 chars) required; `description` (≤ 2000 chars), `start`, `end` (dates) optional. The core widget renders the base fields as today and the extensions **when present** (alert badges colored by severity, sunrise/sunset, UV index) — a provider without them loses nothing, a provider with them needs no custom UI.
