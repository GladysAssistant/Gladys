...
- **at most 4 image commands in flight per integration**, the others queued core-side, and concurrent requests for one key share one command — a grid of 12 posters opened on three tablets costs 12 commands once an hour, never 36 at once; the integration keeps its own download cache and answers from it.
...
| `GET /api/v1/external_integration/:selector/widget/image/:image_key` | 6 |
...
- **Refresh**: on mount, on `expires_at`, on `widget-updated` for its `(selector, key)`, on `websocket.connected`, and on `STATUS_CHANGED` back to `RUNNING`; every fetch coalesced core-side. Device-bound tiles and charts additionally follow `DEVICE.NEW_STATE` like the chips bar.
...
- **Cache and nudge**: hit/miss/TTL expiry, LRU at 50, coalescing of concurrent misses (one command), invalidation on nudge / stop / restart / update / uninstall / successful action, nudge for an undeclared key ignored, rate limit 1 per 10 s per `(integration, key)`, `widget-updated` broadcast.
...
- **Frontend**: Cypress journey with a stubbed widget list and content — the tile appears in the picker, the settings form renders, the widget renders each component type in the canonical slots, each state (stopped, unavailable, check settings, empty) from stubbed responses, a `confirm` button asks before posting.
