...
| --- | --- | --- | --- |
| `external-integration.energy-contract.price` | `{ message_id, contract: { id, template_key, inputs, currency, timezone, billing_period_start_day }, billing_period: { starts_at, ends_at }, cumulative_before: { day, month, billing_period }, intervals: [ { starts_at, kwh, max_power_kw } ] }` (≤ 1,488 intervals = 31 days, all in one billing period; `cumulative_before` = kWh of the priced feature before the first interval, per accumulation scope, computed by the core from the stored states whatever the window start: a live 30-minute run, a catch-up or a recalculation starting mid-period carries the real accumulation, like a first request; from the second period of a request on, the day and period accumulations restart and the month one carries on inside the same local month) | 30 s | `delegated` mode: the integration returns `data.costs: [ { starts_at, cost, components?: { energy, tax } } ]`, one cost per requested interval, in the contract currency. A retry or a catch-up request carries the same state as a first request for the same intervals, so a tiered or monthly-total tariff gives the same answer |
...
- Integration **uninstalled**: `provider_service_id` becomes `NULL` (the foreign keys of `t_energy_contract` and `t_tariff_calendar` are `ON DELETE SET NULL`), the contract stays with the stored `tariff` (a copy taken at import, not a reference) and keeps being computed by the engine in `rules` mode; a `delegated` contract becomes `orphaned`, visible in the UI with a call to action; the catch-up job skips it and the cost job leaves its stored costs untouched (nobody can price it again: a recalculation covering its meter keeps what the integration computed). The uninstall confirmation warns about it when the manifest carries a delegated template.
