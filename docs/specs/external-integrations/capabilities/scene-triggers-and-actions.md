...
"scene_actions": [
  {
    ...
    "outputs": [
      { "key": "image_url", "type": "string", "label": { "en": "Snapshot URL", "fr": "URL de l'instantané" } }
    ]
  }
]
```
...
- `key` (required): `^[a-z0-9_]+$`, ≤ 40 characters, unique among the triggers of the manifest — the identifier the integration fires (§5) and the one stored in the scenes (§4). **Never renamed** once published: a renamed key is a removed key for every scene that uses it (§8);
...
- `fields` (optional, ≤ 10): the **filters** the scene author fills in, same format as the `config_schema` (C.1) with a restricted type list (below). Each field is compared with the event's data of the same key (§4.2). A field left empty by the user is a wildcard; `required: true` denies the wildcard (the Frigate "camera" above: a trigger on "any camera" is rarely what the user means). `default` is honored by the editor, `source: "devices"` is resolved to the integration's devices as everywhere else;
- `variables` (optional, ≤ 20): the data the event exposes to the scene's actions (`{{triggerEvent.data.<key>}}`, §4.3). Per entry: `key` (`^[a-z0-9_]+$`, unique among the variables), `type` (`string` | `number` | `boolean`), `label` (multi-language, `en` required), `description` (optional). **This list is a whitelist**: an event key declared nowhere never enters the scene (§5). A key may be both a field and a variable (the `label` and `zone` above: filterable, and readable by the actions when the filter was a wildcard).
...
- `fields` (optional, ≤ 10): the parameters of the action, same format and same type list as the trigger fields. `required` and `default` follow the `config_schema` semantics; `source: "devices"` passes the chosen `external_id`, exactly like a manifest action (C.1). **`string` fields accept scene variables** (`{{…}}`), resolved by the core before the action is sent (§4.4);
...
this.event.emit(EVENTS.TRIGGERS.CHECK, {
  ...
  data: { camera: 'ext:frigate:front', label: 'person', zone: 'driveway', score: 0.92 }, // whitelisted, see §5
});
```
...
2. for each key of `trigger.fields`, with `expected = trigger.fields[key]` and `actual = event.data[key]`:
   - `expected` is `null`, `undefined`, `''` or `[]` → **wildcard**, the key is skipped;
   - `expected` is an array (`multi_select`) → match iff `actual` is one of its values (`===`);
   - otherwise → match iff `actual === expected` (strict: a `number` field is compared as a number, a `select` value as the string it is; the editor stores the typed value, never a stringified one);
   - `actual` absent while `expected` is set → no match;
3. every key matched → the scene runs, with `{ triggerEvent: event }` as the scope like every trigger.
...
The scope carries the raw event under `triggerEvent`, so a declared variable is `{{triggerEvent.data.<key>}}` — the same mechanism the calendar trigger uses for `{{triggerEvent.calendarEvent.name}}`, and the same Handlebars rendering (`noEscape`) as every templated action field. The editor lists the declared `variables` in the variable picker with their localized labels (§7); only whitelisted keys are ever present in `data` (§5), so the picker and the runtime agree by construction.
...
1. **Templating**: every `string` field of `action.fields` is rendered with Handlebars against the scope (`{{0.0.last_value}}`, `{{triggerEvent.data.zone}}`…), like `mqtt.send`'s `message`. Other types are never templated (a `select` value is an enum member, a `number` is a number).
2. **Resolution**: `self.service.getService(action.integration)` → the proxy service. Absent (uninstalled), or without a `scene` capability (an integration whose manifest declares no `scene_actions`) → the action fails with a clear error (`EXTERNAL_INTEGRATION_NOT_FOUND` / `SCENE_ACTION_NOT_DECLARED`).
3. **Relay**: `proxy.scene.runAction(action.action_key, resolvedFields)` → the supervisor's `runSceneAction(service, key, fields)`:
   - finds the declaration in `service.manifest.scene_actions` (`404`-class error if absent — a key removed by an update, §8);
   - **validates the resolved values** against the declared `fields` with the existing engine (`validateConfigValue` per field, `required`, dynamic `source: "devices"` values resolved like everywhere else) — the same rule as the manifest actions: nothing unvalidated reaches the container, and a stale scene fails loudly in the logs instead of sending garbage;
   - `waitForConnection` (the 15 s window of the message relay, C.4) then `sendCommand(service, SCENE_ACTION_RUN, { key, fields }, { timeoutMs: timeout_seconds * 1000 })`. The window matters here more than anywhere: a `system.start` scene fires while the containers are still booting.
4. **Result**: the ack's `data.outputs` (§6) is **whitelisted** against the declared `outputs` (undeclared keys dropped, non-primitive values dropped, strings capped at 10 000 characters, numbers must be finite) and written into the scope at the action's path with `set(scope, path, outputs, { merge: true })` — the `http.request` / `device.get-value` convention — so the next actions read `{{<column>.<row>.<key>}}`. A missing or non-object `data.outputs` is an empty object.
5. **Failure**: timeout, `success: false`, integration disconnected, validation error → the action throws. `executeAction` treats it like any non-abort error: **logged, the scene continues** — the exact behavior of a `device.set-value` on an unreachable device. A following action reading an output of the failed one gets an empty value. A scene action is **never a condition**: it cannot abort the scene by itself. An integration whose answer should gate the rest of the scene declares an output, and the scene author follows it with the core's `condition.only-continue-if` on that variable — conditions stay generic, nothing to add.

Within a parallel group, a scene action holds the group for at most its `timeout_seconds` (the general rule of parallel actions).
...
- `data` is optional and **flat**: ≤ 30 keys, values `string` (≤ 1000 characters), finite `number`, `boolean` or `null`; any nested object or array → `400 BAD_REQUEST`. The event carries *details*, never a payload to interpret — a snapshot goes through `POST /camera/image`, a state through `POST /state`;
- **whitelist**: the core keeps the keys declared in the trigger's `fields` **or** `variables`, drops every other key silently, and stamps `null` on a declared key that is absent — so `event.data` has exactly the declared shape whatever the integration sent (the `normalizeWeather` doctrine: the payload is never trusted);
- a value whose declared type does not match (`"0.92"` for a `number` variable, a number for a `boolean`) is **coerced when unambiguous** (numeric string → number, `"true"`/`"false"` → boolean) and dropped to `null` otherwise — a filter set on that key then does not match, which is the safe outcome;
- **rate limit: 60 events per minute per integration**, `429 TOO_MANY_REQUESTS` beyond (the same in-memory window as the states' 300/min; a scene is heavier than a state). An integration emitting more is publishing a state, not an event, and the developer documentation says so (§10);
...
**Trigger card** (`triggers/ExternalIntegrationTrigger.jsx`): header "<integration name> · <trigger label>"; body = the declared `fields` rendered by the existing `ConfigField` (the `ActionsCard` precedent on the Configuration screen: the same component, `dynamicOptions.devices` fetched from the integration's devices), with two visible rules: a non-required field shows a "any" hint when empty (the wildcard, §4.2), a required one blocks the save until filled (the editor's standard incomplete-trigger behavior). Below the fields, a **"Variables provided by this trigger"** block lists the declared `variables` with their labels; the card declares them to the editor (`setVariablesTrigger`, name `data.<key>`, so the picker inserts `{{triggerEvent.data.<key>}}`). The collapsed summary (`summary.js`) reads "<integration name>: <trigger label>" plus the non-wildcard filters.

**Action card** (`actions/ExternalIntegrationAction.jsx`): same header and same form engine, with one difference: **`string` fields are rendered by the variables-aware text input** (`TextWithVariablesInjected`, the one `mqtt.send` and `message.send` use) so the user inserts `{{…}}` from the picker instead of typing them. The card declares the `outputs` as variables of its path (`setVariables`), and `VARIABLES_ATTRIBUTES_IN_ACTION` gets an entry covering every string field of the action (`fields.*`), so variable paths are rewritten when steps are reordered — without it, the references silently break on drag.

**Unavailable declarations, rendered honestly and never edited behind the user's back**: a card whose `integration` is not in the list (uninstalled) or whose key is no longer declared (removed by an update, §8) renders a warning line ("provided by an integration that is no longer installed" / "no longer declared by the installed version"), the stored `fields` as read-only key/value rows, and stays deletable. A card whose integration is `STOPPED`/`ERROR` renders a softer warning ("integration stopped: this trigger cannot fire until it runs"). The scene saves as-is in every case: the stale entry is inert (trigger) or fails at execution (action, §4.4), and the user decides.
...
- **Update** replaces the stored manifest wholesale (`externalIntegration.update.js`). A trigger or action key that disappears leaves the scenes untouched (§7): the trigger no longer matches anything (the event key is `404` for the integration too), the action fails at execution with `SCENE_ACTION_NOT_DECLARED`. A field added to a declaration reads as a wildcard / its `default` in existing scenes; a field removed is ignored (extra stored keys are dropped at validation, never an error). Hence the rule of §3: keys are permanent, and a declaration only ever grows — the developer documentation states it (§10).
...
- **Rate**: 60 events/min per integration. A scene started by an event is queued like any scene (the scene manager's queue), so a burst degrades into a delay, never into parallel storms.
- **Loops** (accepted risk, documented): a scene action that makes the integration fire an event bound by the same scene loops through the integration — the core's `alreadyExecutedScenes` guard cannot see it (a new event is a new execution). The rate limit bounds the damage to one event per second on average, and the developer documentation names the pattern to avoid (fire events from *external* causes, never as a consequence of a received action).
...
**Developer documentation** (B.12, website): a page "Scene triggers and actions" with the three doctrines a developer needs — *state vs event* (a value is a feature, a happening is a trigger; the `>`-on-an-event smell), *trigger, not data* (transposed from the webhooks), *keys are forever* (§8) — and the loop pattern to avoid (§9).

## 11. Tests (100% patch coverage)

- **Manifest**: `scene_triggers` / `scene_actions` validated (bounds 1–20, unique keys per list, key regex and length, `fields` ≤ 10 with the restricted type list — `secret`/`oauth2`/`account_link` rejected, `{{port:<name>}}` rejected in sections —, `variables`/`outputs` ≤ 20 with the type enum, `timeout_seconds` bounds and default); a manifest with both fields accepted by the vendored schema **and** the code mirror; the indexer fixtures (repo `integration-store`) cover the same cases.
- **Host API** `POST /scene/event`: the full authentication battery of B.13 (401 without/invalid/user/revoked token); `404` on an undeclared key and on an integration declaring no trigger; `400` on nested data, on a string over 1000 characters, on more than 30 keys; whitelist (undeclared keys dropped, absent declared keys `null`, coercion of a numeric string, non-coercible value dropped); `429` past 60/min; the emitted `EVENTS.TRIGGERS.CHECK` payload shape; **tenant isolation** (A's token cannot fire B's key: the declaration lookup is on the caller's manifest).
- **Trigger matching** (`test/lib/scene/triggers/scene.trigger.externalIntegrationSceneEvent.test.js`, on the `mqttReceived` model): integration and key mismatch; wildcard on `null`/`''`/`[]`/absent; `multi_select` membership; strict equality on number and string; absent data key vs set filter; inactive scene skipped; the scope carries `triggerEvent.data`; the gateway/HomeKit listeners ignore the type.
- **Action** (`test/lib/scene/actions/scene.action.externalIntegrationSceneAction.test.js`, on the `sendMqttMessage` model): string fields templated from the scope, non-string fields untouched; `getService` absent → error logged, scene continues; proxy without `scene` capability → error; outputs written at the path and readable by the next action (`{{0.0.image_url}}`); failure → next action reads an empty value.
- **Supervisor** `runSceneAction`: undeclared key; `required` missing → error; `source: "devices"` value of another integration refused; `waitForConnection` honored; `sendCommand` called with `SCENE_ACTION_RUN` and the per-action timeout; outputs whitelisted (undeclared dropped, string capped, non-finite number dropped, non-object → `{}`); `success: false` and timeout rejected.
...
- **Front** (Cypress, on the `ExternalIntegrationOAuth.cy.js` + `Scene.cy.js` model): intercept `GET /api/v1/external_integration/scene` with a manifest declaring one trigger and one action → the "Integrations" category appears in both pickers → pick the trigger, fill a required `select`, see the variables block → pick the action, insert `{{triggerEvent.data.<key>}}` in a string field → save → the persisted JSON has the §4.1 shapes; then intercept an empty list → the cards render the orphan warning and the scene still saves. `compare-translations` for the core strings.
...
2. New scene: trigger picker → "Integrations" → "Demo event" (subtitle: the integration name); set the `select` filter; the variables block lists the string variable. Action picker → "Integrations" → "Echo"; type "Got {{triggerEvent.data.<key>}}" in the field using the picker; add a `message.send` after it with `{{1.0.<output>}}`. Save.
...
7. From the container, fire 61 events in a minute → the 61st is `429`; send `data` with a nested object → `400`; an undeclared key → `404`.
