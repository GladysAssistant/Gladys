...
A feature published **without** `supported_options` falls back to the full `WATER_HEATER_MODE` catalog bounded by `min`/`max`. That fallback exists for legacy features; it is not the recommended path.
