...
- **Layout mechanics**: the component toggles `body.gladys-sidebar-nav`; global CSS (`style/index.css`) pads the **outer** `#app > .page` only (the dashboard routes nest a `.page` of their own) and offsets `.fixed-bottom` bars (editor footer). On the wallpaper pages (dashboard, new-dashboard, edit-dashboard, and since the settings restyle the settings pages and the profile), `.dashboardBackground` slides back under the rail/top bar (negative margin + equal padding — safe because the image is `background-attachment: fixed`), so the sidebar's backdrop blur frosts the actual scene, not a flat page background. The mobile `padding-bottom` that `.dashboardBackground` carries below 992px exists only to clear the dashboard switcher dock: pages without a `.dashboardHeader` (settings, profile) counter it in their own scope instead of inheriting a dead band at the bottom.
