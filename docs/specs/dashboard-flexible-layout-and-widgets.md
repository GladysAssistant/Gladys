...
- **drag feedback is a custom drag layer** (`EditorDragLayer`, a floating pill with the widget's name following the pointer), with the native preview suppressed (`getEmptyImage`). The native ghost was a lottery: Safari rasterizes a blank image for `backdrop-filter` cards (every glass widget) and the touch backend never had a preview — on those machines dragging looked broken with zero feedback and zero error;
