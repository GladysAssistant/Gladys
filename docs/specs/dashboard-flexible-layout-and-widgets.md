...
- **Layout mechanics**: the component toggles `body.gladys-sidebar-nav`; global CSS (`style/index.css`) pads the **outer** `#app > .page` only (the dashboard routes nest a `.page` of their own) and offsets `.fixed-bottom` bars (editor footer). On the wallpaper pages — the dashboard, its editor, the creation page, and (since the integrations pass in G) every `/dashboard/integration` URL — `.dashboardBackground` slides back under the rail/top bar (negative margin + equal padding — safe because the image is `background-attachment: fixed`), so the sidebar's backdrop blur frosts the actual scene, not a flat page background. The mobile `padding-bottom: 4.5rem` that class also carries is the **dashboard-switcher dock spacer**, not a wallpaper property: it is reclaimed with `:not(:has(.dashboardHeader))` so the pages that do not render the dock don't inherit a blank band, in the negative form so a browser without `:has()` keeps the spacer everywhere rather than letting the dock cover the last dashboard row.
