...
- **edit panel dropdowns never clip**: every react-select of the widget edit forms (and the shared feature/room selectors) sets `menuPlacement="auto"`, so a menu opened near the bottom of the scrollable panel flips upward and stays fully visible instead of being cut by the panel edge;
...
- the **dashboard list** in the editor sidebar is a Horizon glass panel with pill rows (always-visible grab handle, active dashboard accented, name ellipsized): reordering rides the pointer-drag engine with the same placement model as the canvas — insertion indicator line in the exact gap, computed from the pointer position via the shared insertion helpers, no per-row drop targets and no layout shift; a plain click still opens the dashboard's edit page (the engine suppresses only the click that ends a real drag);
...
- **Layout mechanics**: the component toggles `body.gladys-sidebar-nav`; global CSS (`style/index.css`) pads the **outer** `#app > .page` only (the dashboard routes nest a `.page` of their own) and offsets `.fixed-bottom` bars (editor footer). On the wallpaper pages (dashboard, new-dashboard, edit-dashboard, and since the settings restyle the settings pages, the profile and the logged-out pages), `.dashboardBackground` slides back under the rail/top bar (negative margin + equal padding — safe because the image is `background-attachment: fixed`), so the sidebar's backdrop blur frosts the actual scene, not a flat page background (the logged-out pages render no rail at all — `body.gladys-sidebar-nav` is absent — so the slide-back rules simply don't apply there). The mobile `padding-bottom` that `.dashboardBackground` carries below 992px exists only to clear the dashboard switcher dock: pages without a `.dashboardHeader` (settings, profile, the auth scene) counter it in their own scope instead of inheriting a dead band at the bottom. The app-level `Layout` no longer special-cases `/login` with a flex-centering `page-single` wrapper (which shrank the auth scene to its content width): every route gets the block `page-main` wrapper, and `AuthLayout` brings its own `page-single` centering inside it.
