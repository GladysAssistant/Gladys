...
- **Layout mechanics**: the component toggles `body.gladys-sidebar-nav`; global CSS (`style/index.css`) pads the **outer** `#app > .page` only (the dashboard routes nest a `.page` of their own) and offsets `.fixed-bottom` bars (editor footer). On the wallpaper pages (dashboard, its editor, dashboard creation, and the calendar — see the calendar bullet in G), `.dashboardBackground` slides back under the rail/top bar (negative margin + equal padding — safe because the image is `background-attachment: fixed`), so the sidebar's backdrop blur frosts the actual scene, not a flat page background. Its mobile `padding-bottom` exists only to keep the last row clear of the dashboard's fixed switcher dock: a wallpaper page that renders no dock keeps only the safe-area inset (the calendar does).
