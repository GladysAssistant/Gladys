...
- **The "Horizon" glass theme is the one and only dashboard style — imposed, not chosen.** Maintainer decision after seeing it live, in line with the Gladys philosophy: ship the best look for everyone instead of a style picker (a `card_style` column existed during the development of this PR and was removed before release; there is no per-dashboard or per-user style switch). It is a full wall-panel-grade restyle of the dashboard page, not just translucent cards:
  ...
  - **the theme is a shared layer beyond the dashboard**: the settings pages (`front/src/routes/settings/`) sit on the same Horizon scene — `SettingsLayout` applies `glass-theme` plus the default scene, and a scoped `.settings-page` pass (`routes/settings/style.css`) declines the same grammar on the settings-only primitives: the section menu as glass-panel pill rows (the sidebar's grammar), bare `h2–h4.card-header` titles as uppercase micro-labels, rounded translucent form controls (incl. react-select and the file input), borderless tables, quiet tinted alerts outside cards, and the pill button hierarchy (solid ink `btn-primary` CTA, soft pastel color-coded pills, quiet neutral secondary, frosted `btn-outline-primary` capsules);
