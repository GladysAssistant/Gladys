...
- **Layout mechanics**: the component toggles `body.gladys-sidebar-nav`; global CSS (`style/index.css`) pads the **outer** `#app > .page` only (the dashboard routes nest a `.page` of their own) and offsets `.fixed-bottom` bars (editor footer). On the four wallpaper pages (dashboard, editor, new dashboard, and the zone form of Plans), `.dashboardBackground` slides back under the rail/top bar (negative margin + equal padding — safe because the image is `background-attachment: fixed`), so the sidebar's backdrop blur frosts the actual scene, not a flat page background. The scene carries **no** bottom-dock padding: clearing the mobile switcher dock is an opt-in `withBottomDock` class set by the dashboard page alone (the only one with a dock), so the other wallpaper pages don't inherit a dead 4.5rem band — an explicit class rather than `:has(.dashboardHeader)`, whose unknown pseudo-class would invalidate the whole rule, and with it the dock clearance, on exactly the old wall tablets the dock targets.
