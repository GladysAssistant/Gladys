...
- **The "Horizon" glass theme is the one and only dashboard style — imposed, not chosen.** Maintainer decision after seeing it live, in line with the Gladys philosophy: ship the best look for everyone instead of a style picker (a `card_style` column existed during the development of this PR and was removed before release; there is no per-dashboard or per-user style switch). It is a full wall-panel-grade restyle of the dashboard page, not just translucent cards:
  ...
  - **beyond the dashboard — Discussion**: the theme is not dashboard-only any more. The chat page (`routes/chat`) composes on the same global `glass-theme` class and the **default** scene (`glassScene` imported from the dashboard stylesheet — chat has no per-page appearance, so it never goes through `getBackgroundSceneClass`): glass chat card, frosted white bubbles for Gladys against accent-gradient bubbles for the user, nested-glass tool-call rows, transparent composer on the card with a frosted input and pill model selector, full-bleed glass below 768px. The class goes on the page's own nested `.page` so the sidebar (a sibling of the router) is not restyled by it. Two details the theme imposes on any page that joins it: `.glass-theme .card p` colors every card paragraph in ink, so secondary copy (empty state, "Gladys is typing") has to out-specify that rule to stay muted; and `dark-mode.css` paints `.page-main` and `.form-control` with solid `!important` backgrounds, which a glass page must neutralize so its surfaces invert *with* the theme instead of masking the scene;
...
- **Layout mechanics**: the component toggles `body.gladys-sidebar-nav`; global CSS (`style/index.css`) pads the **outer** `#app > .page` only (the dashboard routes nest a `.page` of their own) and offsets `.fixed-bottom` bars (editor footer). On the **four** wallpaper pages, the scene slides back under the rail/top bar (negative margin + equal padding — safe because the scene is `background-attachment: fixed`), so the sidebar's backdrop blur frosts the actual scene, not a flat page background. Three of them (dashboard / edit / new) share `.dashboardBackground`; **Discussion carries its own copy of the slide on `.chatPage`** rather than reusing that class, deliberately: `.dashboardBackground` also always reserves the mobile dock space (`padding-bottom: calc(4.5rem + env(safe-area-inset-bottom))`) for the `.dashboardHeader` pill bar, which the chat never renders — reusing it would open a dead band above the composer on phones.
