...
- `background_scene`: the background is picked from a **catalog of built-in CSS scenes** (migration `20260819100000`; `DASHBOARD_BACKGROUND_SCENE` in `server/utils/constants.js`: `horizon` — the default, stored as `null` — plus `aurora`, `dusk`, `forest`, `lagoon`, `sand`, `lavender`, `mist`). Maintainer decision: a free image URL was the wrong contract for a background — nobody knows what to paste in it, it silently breaks offline (Gladys is local-first) or when the link dies, and it leaks a third-party request from every viewer. Pure-CSS scenes weigh nothing, stay crisp at any resolution and ratio, need no license, and invert cleanly in dark mode. Scene glows are sized in **%** so a scene renders the same composition at every element size — which is what lets the settings-panel **thumbnail picker** preview each scene truthfully (the thumbs carry the scene classes directly; active = accent ring, `role="radio"`, same grammar as the icon picker). The editor previews the picked scene live. The scene map lives in `front/src/routes/dashboard/backgroundScenes.js`, with an unknown-value fallback to the default so a dashboard saved by a newer version never renders bare;
- `background_image` (legacy, kept for compatibility): the earlier free **http(s) URL** contract (validated on save, re-checked by the frontend before interpolation into the CSS `url()`). The URL input was **removed from the UI** — dashboards that already carry one keep rendering it (the image wins over the scene), and picking any scene retires it. The real "my own photo" need points to uploaded backgrounds through a dashboard-asset reference (the `t_dashboard_asset` plumbing from phase 4) as a follow-up PR — offline-safe by construction, unlike a URL. When neither is set, the default built-in scene applies (see below);
- **The "Horizon" glass theme is the one and only dashboard style — imposed, not chosen.** Maintainer decision after seeing it live, in line with the Gladys philosophy: ship the best look for everyone instead of a style picker (a `card_style` column existed during the development of this PR and was removed before release; there is no per-dashboard or per-user style switch). It is a full wall-panel-grade restyle of the dashboard page, not just translucent cards:
  ...
  - **default scene**: when nothing is configured, the default pure-CSS scene (radial glows over a diagonal gradient) is applied — the wow with zero configuration; the picked `background_scene` variant, or a legacy `background_image`, wins over it;
