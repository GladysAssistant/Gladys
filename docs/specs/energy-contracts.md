...
| Field | Source | Note |
| --- | --- | --- |
| `starts_at`, `duration_minutes` | `thirty-minutes-consumption` state (`created_at` − 30 min, as today); `duration_minutes` defaults to 30 and is 1440 for a daily feature | UTC instants |
...
| --- | --- | --- | --- |
| 30-min cost | `0,30 * * * *` | last interval | unchanged; for a `tier` contract, recomputes from the start of the relevant accumulation (day or period) |
| Cost from yesterday | 11:10 and 16:10 local | 24 h | unchanged (late Enedis data) |
| Recalculation on calendar change | on demand, bounded 1 / 10 min / calendar | from `changed_from` | new (capability file, host API) |
| Billing period end | once a day at 02:00, acts when a period has ended | the elapsed period | new: `demand` and closing of the `tier` accumulations |
| `delegated` catch-up | every 30 min with the main job | oldest interval without a cost, ≤ 31 days | new |
...
| `GET /api/v1/device_feature/energy_consumption` | Unchanged on the contract side; the subscription is no longer added on the fly when the contract carries it in its states (§7.2), a `subscription_included: true` field tells the frontend |

The `period_start_day` parameter of the aggregation is still accepted but defaults to the `billing_period_start_day` of the meter's active contract.
...
2. **4-step creation wizard**: (a) meter; (b) template, filtered by country then supplier, with the source visible (catalogue or integration name) and a "propose a contract to the community" link (the existing `contractNotListed*` text, completed with "or publish it as an integration"); (c) template parameters (`inputs`: power, off-peak slots with the existing 48-button grid, region), name, start date; (d) **preview** over the last 7 days of real consumption through `preview`, with the total per component and a curve of the applied prices, then save.
...
4. **Calendars page** (Settings sub-tab): for each calendar, the provider, the coverage and the last 30 values; a "recalculate from" button.
5. **Existing dashboard widget** (`energy-consumption`) unchanged; a new small "electricity price" widget shows the current price, the current tier and the next change (`/current`), for every contract without specific code.
6. **Scenes**: an `energy.price-changed` trigger (the contract moves to another tier) and an `energy.current-price` condition (current price compared to a threshold), fed by the core from the active contract, with no additional integration contract.

**8.3 AI assistant and weekly digest**: the digest prompt and the chat tool read the contract name and the currency from `t_energy_contract` instead of `contract_name`; the digest can name the most expensive tier of the week from the rule labels (optional `label` on a rule).
...
**9.1 Grouping**: one contract row per `(electric_meter_device_id, contract_name, contract, subscribed_power, start_date)`; the group `end_date` = the largest `end_date` (or `NULL`); `currency` `euro` → `EUR`; `timezone` = system timezone; `billing_period_start_day` = the value of the widget's current system variable if it exists, otherwise 1; `provider_kind` = `community` when the name matches a catalogue template, otherwise `user`.
...
**9.3 Verification built into the migration**: after conversion, the core prices the last 7 days of each meter with the old code (kept in a `legacy/` module removed two releases later) and with the new engine; a gap above 0.5% is logged as a warning with the per-day detail and shown on the Contracts page ("check this migrated contract"). No cost state is rewritten by the migration itself: the first job after the update only recomputes its normal window.
...
Five independent PRs, each shippable and testable on its own, the first being this spec (the spec-first process of `AGENTS.md`).
...
PRs 3, 4 and 5 can run in parallel. A pilot for PR 4 is an external "Octopus Energy" integration (`rules` Economy 7 template + 30-min Agile calendar), published in the store as a contributor would: the proof that a contract can ship entirely outside the Gladys repositories.
...
- Migration: `t_energy_price` fixture covering the three types, equality within 0.5% between `legacy/` and the engine over 7 days, identical `GET /energy_price` projection before / after.
...
- Frontend: Cypress on the wizard (creating a peak / off-peak contract from the catalogue, preview shown, contract listed as active); `compare-translations`.
...
- [ ] **Subscription in the cost states or at display time?** The spec puts it in the states (7.2) so every aggregation is consistent; the drawback is that a retroactive subscription change forces a recalculation. To decide before PR 2.
- [ ] **Timezone per contract or per instance?** The spec chooses per contract (Economy 7, Tempo outside France). If deemed superfluous, `timezone` stays in the database with the system value and without a UI field.
- [ ] **`max_price_per_kwh` bound of the delegated mode**: 10 units / kWh fits EUR, USD, CAD, GBP but not JPY nor KRW; propose a per-currency bound (table in the core) rather than a constant.
