...
A consumption interval is priced by the active contract of the root meter at the interval start date. The mode decides the calculation, the output is identical. The window of a run never starts at zero for a `delegated` contract: the core hands the integration the real accumulation of the priced feature before the first interval (`getFeatureCumulative`, the feature's own kWh since the start of the day, month and billing period, like the engine accumulates a feature's own kWh for a `rules` contract), whereas a tiered `rules` contract widens the window to the accumulation boundary instead (7.4). An `orphaned` delegated contract is skipped by the cost job and the intervals a delegated integration cannot price keep their previous cost state, if any: an outage or a recalculation never wipes what the integration already priced.
...
| `POST /api/v1/energy_contract/recalculate` | Body `{ from }`: recompute the costs of every meter carrying a contract from that instant (the calendars page button); a date alone (`YYYY-MM-DD`, what the page sends) is its local midnight in the Gladys timezone, never UTC midnight |
...
| `GET /api/v1/device_feature/energy_consumption` | Unchanged shape: in currency mode the aggregation still prepends one **subscription series** (`deviceFeature.is_subscription: true`, named after the contract) to the energy series, and the `energy-consumption` widget still shows it only when its `show_subscription_prices` option is on. The series is no longer read from `t_energy_price` but computed by `computeFixedCharges` (`server/lib/energy-contract/contract.fixedCharges.js`) from the contracts of the **root meter of the first selector**, once per request whatever the number of devices in the widget: each display period (hour, day, week, month, year, or the billing period when `period_start_day` is not the 1st, the same buckets as the DuckDB aggregation) is cut at every local midnight of the contract active at that instant and priced by the engine with the `consumption` and `demand` kinds excluded, so the pro rata over the real day or month, the period conditions of a `fixed` component, the taxes on it and a contract change inside the period are the engine's; an instant no contract covers costs nothing, a meter whose contracts carry no `fixed` component gets no series, and the series is trimmed to the range of the energy data as before |
...
4. **Calendars section** (Settings tab of the energy-monitoring page): for each calendar, the provider (its manifest name for an integration, `display_name`; or "no provider" when orphaned), the granularity, the coverage (dates for a daily calendar, instants for a 30-minute one) and the last 30 values; a "recalculate the costs from" date button (`POST /energy_contract/recalculate`).
5. **Existing dashboard widget** (`energy-consumption`) unchanged, its "show the subscription" option included (off by default: the user sees the energy alone, a washing-machine plug never shows the subscription of the meter); a new small "electricity price" widget (`energy-price` box, config `{ energy_contract: <selector>, name? }`) shows the current price, the current tier, the next change (its time alone when it falls today, its weekday and date otherwise) and today's kWh (`/current`, refreshed every 5 minutes), for every contract without specific code.
...
- [x] **Subscription in the cost states or at display time?** At display time (8.1), decided on field feedback (24 September 2026): a first version stored it in the states, which made it impossible to show the energy alone (the widget option users relied on) and counted the subscription on every sub-meter of the meter (a washing-machine plug "paid" the whole subscription). The engine still computes it (`computeFixedCharges`, with the exact pro rata), the states stay energy-only and the migration needs no recalculation (9.3).
