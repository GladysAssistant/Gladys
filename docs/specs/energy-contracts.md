...
**9.3 Verification built into the migration**: after conversion, the core prices the last 7 days of each meter with the old code (kept in a `legacy/` module removed two releases later) and with the new engine; a gap above 0.5% is logged as a warning with the per-day detail and shown on the Contracts page ("check this migrated contract"). The migration runs once (a system variable marks it done) at the start of Gladys, after the devices are loaded; it then queues one full cost recalculation of the migrated meters from the earliest `valid_from`, so every stored cost carries the subscription of its contract like the new ones (§7.2) and the display stays consistent over the whole history.
