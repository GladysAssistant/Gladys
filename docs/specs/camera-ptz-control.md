...
**One contract addition is required:** the *discovered device* payload (`POST /discovered_device`, C.3 of `external-integrations/contracts/host-api-endpoints.md`) and the device payload accepted at creation must carry `features[].supported_options` so an ONVIF integration can publish both its supported movements and the preset list it read from the camera (and republish when presets change). The core side (`device.create` → `syncFeatureSupportedOptions`) already supports it; the external-integrations spec, SDK typings (`publishDiscoveredDevices`), and store validator must document/accept the field. **Per the living-spec rule, `docs/specs/external-integrations/` is updated in the same diff as that implementation.**
